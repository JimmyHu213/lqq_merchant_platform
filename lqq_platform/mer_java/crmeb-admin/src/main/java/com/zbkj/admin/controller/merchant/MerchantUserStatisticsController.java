...
@Slf4j
@RestController
@RequestMapping("api/admin/merchant/user/statistics")
@Api(tags = "用户统计-控制器")
public class MerchantUserStatisticsController {

    @Autowired
    private UserStatisticsService userStatisticsService;

    @PreAuthorize("hasAuthority('merchant:user:statistics:top')")
    ...
    public CommonResult<List<StatisticsTopResponse>> topData(@Validated DateRequest dateRequest) {
        return CommonResult.success(userStatisticsService.topMerchantData(dateRequest));
    }

    @PreAuthorize("hasAuthority('merchant:user:statistics:add')")
    ...
    public CommonResult<List<StatisticsPieResponse>> chartUserAddData(@Validated DateRequest dateRequest) {
        return CommonResult.success(userStatisticsService.chartMerchantUserAddData(dateRequest));
    }

    @PreAuthorize("hasAuthority('merchant:user:statistics:alive')")
    ...
    public CommonResult<List<StatisticsPieResponse>> chartUserAliveData(@Validated DateRequest dateRequest) {
        return CommonResult.success(userStatisticsService.chartMerchantUserAliveData(dateRequest));
    }

    @PreAuthorize("hasAuthority('merchant:user:statistics:member')")
    ...
    public CommonResult<List<StatisticsPieResponse>> chartUserMemberData(@Validated DateRequest dateRequest) {
        return CommonResult.success(userStatisticsService.chartMerchantUserMemberData(dateRequest));
    }

    @PreAuthorize("hasAuthority('merchant:user:statistics:bought')")
    ...
    public CommonResult<List<UserStatisticsChartResponse>> chartUserBoughtData(@Validated DateRequest dateRequest) {
        return CommonResult.success(userStatisticsService.chartMerchantUserBoughtData(dateRequest));
    }
}
