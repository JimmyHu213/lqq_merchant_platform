...
//                .globalOperationParameters(pars) // 针对单个url的验证 如果需要的话
    ...
    public Docket create4RestApis() {
        return new Docket(DocumentationType.SWAGGER_2)
                ...
                .select()
                // 扫描的路径包
                .apis(RequestHandlerSelectors.basePackage("com.zbkj.admin.controller.circle"))
                // 指定路径处理PathSelectors.any()代表所有的路径
                .paths(circlePathsAnt())
                .build()
                .securitySchemes(security())
                .securityContexts(securityContexts())
//                .globalOperationParameters(pars) // 针对单个url的验证 如果需要的话
    ...
    private Predicate<String> circlePathsAnt() {
        return PathSelectors.ant("/api/admin/circle/**");
    }
