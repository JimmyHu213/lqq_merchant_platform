...
public class PlatformExportController {
    ...
    @PreAuthorize("hasAuthority('platform:export:user:excel')")
    ...
    public CommonResult<HashMap<String, String>> exportUser(@Validated UserSearchRequest request){
        String fileName = exportService.exportUser(request);
        HashMap<String, String> map = CollUtil.newHashMap();
        map.put("fileName", fileName);
        return CommonResult.success(map);
    }

    @PreAuthorize("hasAuthority('platform:export:retail:store:people:excel')")
    @ApiOperation(value = "导出分销员Excel")
    @RequestMapping(value = "/retail/store/people/excel", method = RequestMethod.GET)
    public CommonResult<HashMap<String, String>> exportRetailStorePeople(@Validated RetailStorePeopleSearchRequest request){
        String fileName = exportService.exportRetailStorePeople(request);
        HashMap<String, String> map = CollUtil.newHashMap();
        map.put("fileName", fileName);
        return CommonResult.success(map);
    }
