...
@Slf4j
@RestController
@RequestMapping("api/admin/circle/product/reply")
@Api(tags = "商品评论控制器") //配合swagger使用
public class CircleProductReplyController {

    @Autowired
    private CircleProductReplyService circleProductReplyService;

    @PreAuthorize("hasAuthority('circle:product:reply:list')")
    ...
    public CommonResult<CommonPage<ProductReplyResponse>> getList(@Validated ProductReplySearchRequest request) {
        return CommonResult.success(CommonPage.restPage(circleProductReplyService.getCircleProductReplyPage(request)));
    }
