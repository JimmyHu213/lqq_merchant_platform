...
@Slf4j
@RestController
@RequestMapping("api/admin/platform/product/statistics")
@Api(tags = "商品统计-控制器")
public class ProductStatisticsController {
    ...
    @PreAuthorize("hasAuthority('platform:product:statistics:top')")
    ...
    public CommonResult<List<StatisticsTopResponse>> topData(@Validated DateRequest dateRequest) {
        return CommonResult.success(productStatisticsService.topPlatformData(dateRequest));
    }

    @PreAuthorize("hasAuthority('platform:product:statistics:chart')")
    ...
    public CommonResult<List<ProductStatisticsChartResponse>> chartData(@Validated DateRequest dateRequest) {
        return CommonResult.success(productStatisticsService.chartPlatformData(dateRequest));
    }

    @PreAuthorize("hasAuthority('platform:product:statistics:category')")
    @ApiOperation(value = "商品分类统计")
    @RequestMapping(value = "/category", method = RequestMethod.GET)
    public CommonResult<List<StatisticsPieResponse>> categoryData() {
        return CommonResult.success(productStatisticsService.categoryPlatformData());
    }

    @PreAuthorize("hasAuthority('platform:product:statistics:type')")
    @ApiOperation(value = "商品类型统计")
    @RequestMapping(value = "/type", method = RequestMethod.GET)
    public CommonResult<List<StatisticsPieResponse>> typeData() {
        return CommonResult.success(productStatisticsService.typePlatformData());
    }
