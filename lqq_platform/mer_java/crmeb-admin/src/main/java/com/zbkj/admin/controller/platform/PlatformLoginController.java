...
public class PlatformLoginController {
    ...
    public CommonResult<Integer> accountDetection(@RequestBody @Validated AdminAccountDetectionRequest request) {
        return CommonResult.success(loginService.accountDetection(request.getAccount(), RoleEnum.PLATFORM_ADMIN.getValue()));
    }
    ...
    @PreAuthorize("hasAuthority('platform:login:admin:update:password')")
