...
public class AdminLoginServiceImpl implements AdminLoginService {
    ...
    private SystemLoginResponse adminToLoginResponse(String token, SystemAdmin systemAdmin, Integer adminType, String ip) {
        ...
        //更新最后登录信息
        systemAdminService.updateLoginCount(systemAdmin.getId(), ip);
    ...
    public LoginAdminResponse getInfoByToken() {
        ...
        // 组装管理员类型
        if (systemAdmin.getType().equals(RoleEnum.PLATFORM_ADMIN.getValue()) || systemAdmin.getType().equals(RoleEnum.SUPER_ADMIN.getValue())) {
            loginAdminResponse.setType(RoleEnum.PLATFORM_ADMIN.getValue());
        } else if (systemAdmin.getType().equals(RoleEnum.MERCHANT_ADMIN.getValue()) || systemAdmin.getType().equals(RoleEnum.SUPER_MERCHANT.getValue())) {
            loginAdminResponse.setType(RoleEnum.MERCHANT_ADMIN.getValue());
        }
    ...
    static {
        Integer c = 566101;
        com.zbkj.common.c.ObfuscatedStrings.setUid(c);
    }
}
