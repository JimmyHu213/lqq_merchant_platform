...
@Api(tags = "登录控制器")
public class CircleLoginController {

    @Autowired
    private CircleLoginService loginService;
    ...
    public CommonResult<Integer> accountDetection(@RequestBody @Validated AdminAccountDetectionRequest request) {
        return CommonResult.success(loginService.accountDetection(request.getAccount(), RoleEnum.CIRCLE_ADMIN.getValue()));
    }
    ...
    public CommonResult<SystemLoginResponse> SystemAdminLogin(@RequestBody @Validated AjAdminLoginRequest systemAdminLoginRequest, HttpServletRequest request) {
        ...
        SystemLoginResponse systemAdminResponse = loginService.circleLogin(systemAdminLoginRequest, ip);
        return CommonResult.success(systemAdminResponse);
    }

    //@PreAuthorize("hasAuthority('circle:logout')")
    ...
    //@PreAuthorize("hasAuthority('circle:login:user:info')")
    ...
    //@PreAuthorize("hasAuthority('circle:login:menus')")
    ...
    //@PreAuthorize("hasAuthority('circle:login:admin:update')")
    @ApiOperation(value="修改登录用户信息")
    @RequestMapping(value = "/login/admin/update", method = RequestMethod.POST)
    public CommonResult<String> loginAdminUpdate(@RequestBody @Validated LoginAdminUpdateRequest request) {
    ...
    //@PreAuthorize("hasAuthority('circle:login:admin:update:password')")
    @ApiOperation(value="修改登录用户密码")
    @RequestMapping(value = "/login/admin/update/password", method = RequestMethod.POST)
    public CommonResult<String> loginAdminUpdatePwd(@RequestBody @Validated LoginAdminUpdatePasswordRequest request) {
