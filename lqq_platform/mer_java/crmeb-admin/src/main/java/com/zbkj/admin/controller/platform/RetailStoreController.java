...
public class RetailStoreController {
    ...
    @PreAuthorize("hasAuthority('platform:retail:store:base:config:get')")
    @ApiOperation(value = "分销基础配置信息获取")
    @RequestMapping(value = "/base/config/get", method = RequestMethod.GET)
    public CommonResult<RetailStoreBaseConfigVo> getBaseConfig() {
        return CommonResult.success(retailStoreService.getRetailStoreBaseConfig());
    }

    @PreAuthorize("hasAuthority('platform:retail:store:base:config:save')")
    @ApiOperation(value = "分销基础配置信息保存")
    @RequestMapping(value = "/base/config/save", method = RequestMethod.POST)
    public CommonResult<Object> saveBaseConfig(@RequestBody @Validated RetailStoreBaseConfigVo retailShopRequest) {
        return CommonResult.success(retailStoreService.saveRetailStoreBaseConfig(retailShopRequest));
    }

    @PreAuthorize("hasAuthority('platform:retail:store:extract:config:get')")
    @ApiOperation(value = "分销提现配置信息获取")
    @RequestMapping(value = "/extract/config/get", method = RequestMethod.GET)
    public CommonResult<RetailStoreExtractConfigVo> getExtractConfig() {
        return CommonResult.success(retailStoreService.getRetailStoreExtractConfig());
    }

    @PreAuthorize("hasAuthority('platform:retail:store:extract:config:save')")
    @ApiOperation(value = "分销提现配置信息保存")
    @RequestMapping(value = "/extract/config/save", method = RequestMethod.POST)
    public CommonResult<Object> saveExtractConfig(@RequestBody @Validated RetailStoreExtractConfigVo retailShopRequest) {
        return CommonResult.success(retailStoreService.saveRetailStoreExtractConfig(retailShopRequest));
    }
