...
//                    }
    ...
    public static Boolean isMatch(String dateFormat, String dateStr) {
        try {
            DateTimeFormatter DATE_FORMATTER = DateTimeFormatter.ofPattern(dateFormat);
    ...
    public static Boolean isMatchDate(String dateStr) {
        return isMatch(DateConstants.DATE_FORMAT_DATE, dateStr);
    }

    public static void validateDateLimitFormatDate(String dateLimit) {
        List<String> dateStrList = CrmebUtil.stringToArrayStr(dateLimit);
        dateStrList.forEach(date -> {
            if (!isMatchDate(date)) {
                throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "日期格式不正确！");
            }
        });
    }
     ...
     * @return 转换后的时间字符串
     */
    public static String minutesToHoursMinutes(long totalMinutes) {
        if (totalMinutes < 0) {
            throw new IllegalArgumentException("输入分钟数不能为负数");
        }
    ...
    /**
     * 根据起始日期返回分组键
     * (≤92天)：按日期分组 (yyyy-MM-dd)
     * (≤367天)：按月份分组 (yyyy-MM)
     * (>367天)：按年份分组 (yyyy)
     */
    public static List<String> groupDatesStream(String startDateStr, String endDateStr) {
        ...
        if (daysBetween <= 92) {
            // 按天分组 - 使用IntStream
            return IntStream.range(0, (int) (daysBetween + 1))
                    .mapToObj(startDate::plusDays)
                    .map(LocalDate::toString)
                    .collect(Collectors.toList());
        } else if (daysBetween <= 367) {
            // 按月分组
            // 计算月份差
            ...
            return LongStream.rangeClosed(0, monthsBetween)
                    .mapToObj(startDate.withDayOfMonth(1)::plusMonths)
            ...
            return LongStream.rangeClosed(0, yearsBetween)
                    .mapToObj(startDate.withDayOfYear(1)::plusYears)
                    .map(date -> String.format("%04d", date.getYear()))
                    .distinct()
                    .sorted()
                    .collect(Collectors.toList());
        }
