...
//    private RedisTemplate<String, Object> redisTemplate;
     ...
     * 唯一推送
     */
    public Long lPushUnique(String key, Object data) {
        String setKey = key + ":set";
        String setValue = String.valueOf(data);
        try {
            // 如果已存在，从List中删除旧值
            if (Boolean.TRUE.equals(getRedisTemplate().opsForSet().isMember(setKey, setValue))) {
                getRedisTemplate().opsForList().remove(key, 1, setValue);
            } else {
                // 添加到Set
                getRedisTemplate().opsForSet().add(setKey, setValue);
            }
            // 添加到List头部
            return getRedisTemplate().opsForList().leftPush(key, setValue);
        } catch (Exception e) {
            e.printStackTrace();
            return 0L;
        }
     ...
     * 从右边弹出唯一值
     */
    public Object getRightPopUnique(String k, Long t) {
        String setKey = k + ":set";
        try {
            // 从List右侧弹出
            Object data = getRedisTemplate().opsForList().rightPop(k, t, TimeUnit.SECONDS);
            if (data != null) {
                // 从Set中删除
                getRedisTemplate().opsForSet().remove(setKey, data);
            }
            return data;
        } catch (Exception e) {
            e.printStackTrace();
