...
public class CouponServiceImpl extends ServiceImpl<CouponDao, Coupon> implements CouponService {
    ...
    public PageInfo<CouponFrontResponse> getH5List(CouponFrontSearchRequest request, PageParamRequest pageParamRequest) {
        ...
        PageInfo<Coupon> pageInfo = getH5ListBySearch(request.getIdentityType(), request.getCategory(), request.getMerId(), request.getProductId(), pageParamRequest);
    ...
    public PageInfo<Coupon> getMerchantPageList(CouponSearchRequest request) {
        ...
        if (ObjectUtil.isNotNull(request.getIdentityType())) {
            lqw.eq(Coupon::getIdentityType, request.getIdentityType());
        }
     ...
     * @return List<Coupon>
     */
    private PageInfo<Coupon> getH5ListBySearch(Integer identityType, Integer category, Integer merId, Integer productId, PageParamRequest pageParamRequest) {
     ...
     */
    @Override
    public List<CouponFrontResponse> findMerchantMemberCouponByIds(String ids) {
        List<Integer> idList;
        try {
        ...
        LambdaQueryWrapper<Coupon> lqw = Wrappers.lambdaQuery();
        lqw.in(Coupon::getId, idList);
        lqw.eq(Coupon::getIdentityType,1);
        lqw.eq(Coupon::getPublisher, 2);
        lqw.eq(Coupon::getReceiveType, 1);
        lqw.and(i -> i.eq(Coupon::getIsTimeReceive, 0).or(o -> o.le(Coupon::getReceiveStartTime, now).ge(Coupon::getReceiveEndTime, now)));
        lqw.and(i -> i.eq(Coupon::getIsFixedTime, 0).or(o -> o.ge(Coupon::getUseEndTime, now)));
        lqw.eq(Coupon::getStatus, 1);
        lqw.eq(Coupon::getIsDel, 0);
        ...
        Integer userId = userService.getUserId();
        return couponListToFrontResponse(couponList, userId);
    }
     ...
     */
    @Override
    public List<ProductCouponUseResponse> findMemberUsableList() {
        SystemAdmin systemAdmin = SecurityUtil.getLoginUserVo().getUser();
        String now = DateUtil.now();
        //带 Coupon 类的多条件查询
        LambdaQueryWrapper<Coupon> lqw = new LambdaQueryWrapper<>();
