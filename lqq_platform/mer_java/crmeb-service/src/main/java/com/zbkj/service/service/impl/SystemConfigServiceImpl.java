...
public class SystemConfigServiceImpl extends ServiceImpl<SystemConfigDao, SystemConfig> implements SystemConfigService {
    ...
    public void loadingConfigCache() {
        ...
        lqw.eq(SystemConfig::getMerId, 0);
...
//            query.select("name", "ANY_VALUE(value)", "MAX(id) as id");
    ...
    public Boolean updateOrSaveValueByName(String name, String value) {
        return updateOrSaveValueByName(name, value, 0);
    }
     ...
     */
    @Override
    public Boolean updateOrSaveValueByName(String name, String value, Integer merId) {
        ...
        lambdaQueryWrapper.eq(SystemConfig::getMerId, merId);
        ...
        if (systemConfigs.size() >= 2) {
            ...
            systemConfig.setValue(value);
            systemConfig.setMerId(merId);
            systemConfig.setUpdateTime(DateUtil.date());
            boolean update = updateById(systemConfig);
            if (update && crmebConfig.getAsyncConfig() && merId > 0) {
                async(systemConfig);
            }
            return update;
        } else {
            SystemConfig systemConfig = new SystemConfig().setName(name).setValue(value).setMerId(merId);
            boolean save = save(systemConfig);
            if (save && crmebConfig.getAsyncConfig() && merId > 0) {
                async(systemConfig);
            }
            return save;
        }
    ...
    public String getAgreementByKey(String agreementName) {
        return getAgreementByKey(agreementName, 0);
    }
     ...
     */
    @Override
    public String getAgreementByKey(String agreementName, Integer merId) {
