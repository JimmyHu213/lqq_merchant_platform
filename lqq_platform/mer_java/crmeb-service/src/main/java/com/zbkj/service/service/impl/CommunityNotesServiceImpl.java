...
public class CommunityNotesServiceImpl extends ServiceImpl<CommunityNotesDao, CommunityNotes> implements CommunityNotesService {
    ...
    public PageInfo<Integer> findAuthorPageTimeDesc(Integer userId, PageParamRequest request) {
        ...
        Page<CommunityNotes> page = PageHelper.startPage(request.getPage(), request.getLimit());
        QueryWrapper<CommunityNotes> query = Wrappers.query();
        query.select("uid", "MAX(create_time) as create_time");
        query.notIn("uid", authorConcernedList);
        query.eq("audit_status", CommunityConstants.COMMUNITY_NOTE_AUDIT_SUCCESS);
        query.eq("is_del", Constants.COMMON_IS_FILED_ZERO);
        query.groupBy("uid");
        query.orderByDesc("create_time");
        List<CommunityNotes> list = dao.selectList(query);
