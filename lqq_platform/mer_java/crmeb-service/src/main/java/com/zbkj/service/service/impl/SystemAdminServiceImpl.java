...
public class SystemAdminServiceImpl extends ServiceImpl<SystemAdminDao, SystemAdmin> implements SystemAdminService {
    ...
    public PageInfo<SystemAdminResponse> getList(SystemAdminRequest request, PageParamRequest pageParamRequest) {
        Page<SystemAdmin> page = PageHelper.startPage(pageParamRequest.getPage(), pageParamRequest.getLimit());
        ...
        lqw.eq(SystemAdmin::getIsDel, Boolean.FALSE);
    ...
    public Boolean removeAdmin(Integer id) {
        ...
        if (perDelAdmin.getType().equals(RoleEnum.SUPER_ADMIN.getValue())
                ...
                || perDelAdmin.getType().equals(RoleEnum.CIRCLE_SUPER.getValue())
                || perDelAdmin.getType().equals(RoleEnum.CIRCLE_SUPER.getValue())) {
    ...
    public Boolean updateStatus(Integer id, Boolean status) {
        ...
        if (systemAdmin.getType().equals(RoleEnum.SUPER_ADMIN.getValue())
                ...
                || systemAdmin.getType().equals(RoleEnum.CIRCLE_SUPER.getValue())
                || systemAdmin.getType().equals(RoleEnum.CIRCLE_SUPER.getValue())) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "系统内置权限，不允许编辑");
        }
    ...
    public Boolean updatePassword(SystemAdminUpdatePwdRequest request) {
        ...
        if (adminDetail.getType().equals(RoleEnum.SUPER_ADMIN.getValue())
                || adminDetail.getType().equals(RoleEnum.SUPER_MERCHANT.getValue())
                || adminDetail.getType().equals(RoleEnum.CIRCLE_SUPER.getValue())
                || adminDetail.getType().equals(RoleEnum.CIRCLE_SUPER.getValue())) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "系统内置权限，不允许编辑");
        }
        if (!currentUser.getMerId().equals(adminDetail.getMerId())) {
     ...
     */
    @Override
    public Boolean updateLoginCount(Integer adminId, String ip) {
        LambdaUpdateWrapper<SystemAdmin> wrapper = Wrappers.lambdaUpdate();
        wrapper.setSql("login_count = login_count + 1");
        wrapper.set(SystemAdmin::getLastIp, ip);
        wrapper.set(SystemAdmin::getUpdateTime, DateUtil.date());
        wrapper.eq(SystemAdmin::getId, adminId);
