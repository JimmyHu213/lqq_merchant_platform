...
public class PageLayoutServiceImpl implements PageLayoutService {
     ...
     */
    @Override
    public List<BottomNavigationVo> getMerchantBottomNavigation() {
        SystemAdmin admin = SecurityUtil.getLoginUserVo().getUser();
        List<GroupConfig> configList = groupConfigService.findByTag(GroupConfigConstants.TAG_BOTTOM_NAVIGATION, admin.getMerId(), Constants.SORT_ASC, null);
        if (CollUtil.isEmpty(configList)) {
            return new ArrayList<>();
        }
        return groupConfigToBottomNavigation(configList);
    }

    private List<BottomNavigationVo> groupConfigToBottomNavigation(List<GroupConfig> configList) {
     ...
     */
    @Override
    public Boolean merchantBottomNavigationSave(List<BottomNavigationVo> voList) {
        SystemAdmin admin = SecurityUtil.getLoginUserVo().getUser();
        List<GroupConfig> groupConfigList = new ArrayList<>();
        String cdnUrl = systemAttachmentService.getCdnUrl();
        voList.forEach(vo -> {
            GroupConfig groupConfig = new GroupConfig();
            groupConfig.setTag(GroupConfigConstants.TAG_BOTTOM_NAVIGATION);
            groupConfig.setMerId(admin.getMerId());
            groupConfig.setName(vo.getName());
            groupConfig.setLinkUrl(systemAttachmentService.clearPrefix(vo.getSelectedIconLinkUrl(), cdnUrl));
            groupConfig.setImageUrl(systemAttachmentService.clearPrefix(vo.getUnselectedIconLinkUrl(), cdnUrl));
            groupConfig.setValue(systemAttachmentService.clearPrefix(vo.getLinkUrl(), cdnUrl));
        ...
        return transactionTemplate.execute(e -> {
            groupConfigService.deleteByTag(GroupConfigConstants.TAG_BOTTOM_NAVIGATION, admin.getMerId());
            if (CollUtil.isNotEmpty(groupConfigList)) {
                groupConfigService.saveList(groupConfigList);
            }
            return Boolean.TRUE;
        });
    }
