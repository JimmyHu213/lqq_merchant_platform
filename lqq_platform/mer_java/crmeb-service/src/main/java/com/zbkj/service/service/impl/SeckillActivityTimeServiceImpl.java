...
public class SeckillActivityTimeServiceImpl extends ServiceImpl<SeckillActivityTimeDao, SeckillActivityTime> implements SeckillActivityTimeService {
    ...
    public List<Integer> findActivityByDateTime(Integer dateInt, Integer timeInt) {
        QueryWrapper<SeckillActivityTime> query = Wrappers.query();
        query.select("ANY_VALUE(id) as id", "seckill_id");
        query.le("start_date", dateInt);
        query.ge("end_date", dateInt);
        query.le("start_time", timeInt);
        query.ge("end_time", timeInt);
        query.groupBy("seckill_id");
        List<SeckillActivityTime> timeList = dao.selectList(query);
