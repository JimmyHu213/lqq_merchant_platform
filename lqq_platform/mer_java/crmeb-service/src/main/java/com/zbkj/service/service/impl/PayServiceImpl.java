...
public class PayServiceImpl implements PayService {
     ...
     */
    @Override
    public PayConfigResponse getPayConfig(String orderNo) {
        ...
        if (StrUtil.isBlank(orderNo) || orderNo.equals("other")) return response;

        List<MerchantOrder> merchantOrderList = merchantOrderService.getByOrderNo(orderNo);
        if (merchantOrderList.size() > 1) return response;
        Integer userId = userService.getUserIdException();
        Integer merId = merchantOrderList.get(0).getMerId();
        MerchantMember merchantMember = merchantMemberService.getByUidAndMerId(userId, merId);
        if (ObjectUtil.isNotNull(merchantMember) && merchantMember.getFinancialStatus().equals(0)) return response;

        UserShoppingCredits userShoppingCredits = userShoppingCreditsService.getByUserIdAndMerId(userId, merId);
        if (ObjectUtil.isNull(userShoppingCredits)) return response;

        Order order = orderService.getByOrderNo(orderNo);
        BigDecimal amount = userShoppingCredits.getRechargeAmount().add(userShoppingCredits.getGiftAmount());
        if (order.getPayPrice().compareTo(amount) <= 0) {
            response.setShoppingCreditsStatus(true);
            response.setShoppingCreditsBalance(amount);
        }
        return response;
    }
    ...
    public OrderPayResultResponse payment(OrderPayRequest orderPayRequest) {
        ...
        if (order.getPayType().equals(PayConstants.PAY_TYPE_SHOPPING_CREDITS)) {
            Boolean payResult = shoppingCreditsPay(order, user);
            response.setStatus(payResult);
            return response;
        }
...
//            response.setJsConfig(vo);
    ...
    public Boolean queryAliPayResult(String orderNo) {
        ...
        if (PayConstants.PAY_SERVICE_TYPE_SVIP.equals(orderType)) {// VIP订单
        ...
        if (PayConstants.PAY_SERVICE_TYPE_SHOPPING_CREDITS.equals(orderType)) {// 购物金订单
            ShoppingCreditsOrder shoppingCreditsOrder = shoppingCreditsOrderService.getByOutTradeNo(orderNo);
            if (ObjectUtil.isNull(shoppingCreditsOrder)) {
                throw new CrmebException(StrUtil.format("ali pay query error : 购物金订单后置处理，没有找到对应订单，支付服务方订单号：{}", orderNo));
            }
            ...
            aliPayQuery(orderNo);
            // 支付成功处理
            Boolean shoppingCreditsPayAfter = shoppingCreditsOrderService.paySuccessAfter(shoppingCreditsOrder);
            if (!shoppingCreditsPayAfter) {
                throw new CrmebException(StrUtil.format("ali pay shopping credits pay after error : 数据保存失败==》" + orderNo));
            }
            return Boolean.TRUE;
        }
    ...
    public Boolean payAfterProcessingTemp(String orderNo) {
        ...
        List<MerchantMember> merchantMemberList = new ArrayList<>();
        ...
        for (Order order : orderList) {
            ...
            if (merchantOrder.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_RESERVATION) && merchantOrder.getShippingType().equals(OrderConstants.ORDER_SHIPPING_TYPE_HOME_SERVICE)) {
                ...
                for (int i = 0; i < orderDetail.getPayNum(); i++) {
                    ...
                    try {
                        ...
                        Location location = AddressUtil.getLocationByTx(txKey, merchantOrder.getUserAddress());
                        workOrder.setLongitude(location.getLongitude());
                        workOrder.setLatitude(location.getLatitude());
                    } catch (Exception e) {
                        logger.error("通过腾讯地图，解析地址获取经纬度失败！orderNo = {}", orderNo);
                        logger.error("通过腾讯地图，解析地址获取经纬度失败！", e);
                    }
            ...
            // 次卡订单有效期处理，将订单号放入队列中，由定时任务处理有效期逻辑
            if (merchantOrder.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CARDTIMES)) {
                redisUtil.lPush(TaskConstants.CARD_TIMES_ORDER_VALIDITY, order.getOrderNo());
            }

            MerchantMember merchantMember = merchantMemberService.getByUidAndMerId(order.getUid(), order.getMerId());
            if (ObjectUtil.isNotNull(merchantMember) && !merchantMember.getIsLogoff()) {
                merchantMemberList.add(merchantMember);
            }
        }
        ...
        if (CollUtil.isNotEmpty(merchantMemberList)) {
            for (int i = 0; i < merchantMemberList.size(); ) {
                MerchantMember merchantMember = merchantMemberList.get(i);
                BigDecimal consumptionAmount = orderDetailService.getConsumptionAmountByMerAndUid(merchantMember.getMerId(), merchantMember.getUid());
                List<MerchantMemberLevel> memberLevelList = merchantMemberLevelService.getList(merchantMember.getMerId());
                MerchantMemberLevel userMemberLevel = getUserReasonableLevel(consumptionAmount, memberLevelList);
                if (ObjectUtil.isNull(userMemberLevel) || userMemberLevel.getLevel().equals(merchantMember.getLevel())) {
                    merchantMemberList.remove(i);
                    continue;
                }
                merchantMember.setLevel(userMemberLevel.getLevel());
                merchantMember.setUpdateTime(new Date());
                if (StrUtil.isNotBlank(userMemberLevel.getCouponIds())) {
                    String couponIds = userMemberLevel.getCouponIds();
                    ...
                    if (CollUtil.isNotEmpty(memberCouponList)) {
                        couponList.addAll(memberCouponList);
                    }
                }
                i++;
            }
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            if (CollUtil.isNotEmpty(merchantMemberList)) {
                merchantMemberService.updateBatchById(merchantMemberList);
            }
            if (CollUtil.isNotEmpty(couponList)) {
                couponUserService.batchReceiveCoupon(couponList, user.getId());
            }
            return Boolean.TRUE;
        });
        if (execute) {
    ...
    public Boolean queryWechatPaymentResult(WechatPaymentQueryRequest request) {
        ...
        if (request.getOrderType().equals(PayConstants.PAY_SERVICE_TYPE_SHOPPING_CREDITS)) {
            return queryShoppingCreditsWechatPayResult(request.getOrderNo());
        }
     ...
     */
    @Override
    public Boolean queryWechatTransferResult(String orderNo) {
        TransferBillsGetResult transferBillsGetResult = wechatPayService.queryWechatTransferResult(orderNo);
        logger.info("--------------------------------------------------------------------------------");
        logger.info("transferBillsGetResult = " + JSONObject.toJSONString(transferBillsGetResult));
        if ("SUCCESS".equals(transferBillsGetResult.getState())) {
            UserClosing userClosing = userClosingService.getByClosingNo(orderNo);
            if (userClosing.getAccountStatus().equals(1)) {
                throw new CrmebException(CommonResultCode.SUCCESS, "微信商家转账已成功");
            }
            userClosing.setAccountStatus(1);
            userClosing.setClosingTime(DateUtil.date());
            userClosing.setUpdateTime(DateUtil.date());
            return userClosingService.updateById(userClosing);
        }
        return false;
    }

    private Boolean queryShoppingCreditsWechatPayResult(String orderNo) {
        ShoppingCreditsOrder shoppingCreditsOrder = shoppingCreditsOrderService.getByOrderNo(orderNo);
        if (ObjectUtil.isNull(shoppingCreditsOrder)) {
            throw new CrmebException(OrderResultCode.ORDER_NOT_EXIST);
        }
        if (!shoppingCreditsOrder.getPayType().equals(PayConstants.PAY_TYPE_WE_CHAT)) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "不是微信支付类型订单");
        }
        if (shoppingCreditsOrder.getPaid()) {
            return Boolean.TRUE;
        }
        WechatPayInfo wechatPayInfo = wechatPayInfoService.getByNo(shoppingCreditsOrder.getOutTradeNo());
        if (ObjectUtil.isNull(wechatPayInfo)) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "未找到对应微信订单");
        }
        String paySource = getWechatPaySource(shoppingCreditsOrder.getPayChannel());
        WxPayOrderQueryResult queryResult = wechatPayService.queryOrder(shoppingCreditsOrder.getOutTradeNo(), paySource);
        if (!queryResult.getTradeState().equals("SUCCESS")) {
            return Boolean.FALSE;
        }
        wechatPayInfo.setIsSubscribe(queryResult.getIsSubscribe());
        ...
        wechatPayInfo.setTradeStateDesc(queryResult.getTradeStateDesc());
        Boolean updatePaid = transactionTemplate.execute(e -> {
            Boolean update = shoppingCreditsOrderService.paySuccessAfter(shoppingCreditsOrder);
            if (!update) {
                logger.error("购物金订单支付处理结果失败，orderNo = {}", shoppingCreditsOrder.getOrderNo());
                e.setRollbackOnly();
                return false;
            }
            wechatPayInfoService.updateById(wechatPayInfo);
        ...
        if (!updatePaid) {
            throw new CrmebException("购物金支付成功更新订单失败");
        }
        return Boolean.TRUE;
    }
    ...
    private List<UserBrokerageRecord> assignCommission(MerchantOrder merchantOrder, List<OrderDetail> orderDetailList) {
        ...
        if (spreadRecordList.size() == 1) {
            ...
            for (OrderDetail orderDetail : orderDetailList) {
                ...
                BigDecimal detailPrice = (orderDetail.getIsSvip() && orderDetail.getVipPriceType().equals(1)) ? orderDetail.getVipPrice() : orderDetail.getPrice();
...
//        integralRecord.setBalance(balance);
     ...
     * 购物金支付
     */
    private Boolean shoppingCreditsPay(Order order, User user) {
        MerchantOrder merchantOrder = merchantOrderService.getOneByOrderNo(order.getOrderNo());
        UserShoppingCredits userShoppingCredits = userShoppingCreditsService.getByUserIdAndMerId(order.getUid(), merchantOrder.getMerId());

        if ((userShoppingCredits.getRechargeAmount().add(userShoppingCredits.getGiftAmount())).compareTo(order.getPayPrice()) < 0) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "用户购物金不足");
        }

        BigDecimal rechargeAmount;
        BigDecimal giftAmount;
        if (userShoppingCredits.getRechargeAmount().compareTo(order.getPayPrice()) >= 0) {
            rechargeAmount = order.getPayPrice();
            giftAmount = BigDecimal.ZERO;
        } else {
            rechargeAmount = userShoppingCredits.getRechargeAmount();
            giftAmount = order.getPayPrice().subtract(userShoppingCredits.getRechargeAmount());
        }
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            order.setPayType(PayConstants.PAY_TYPE_SHOPPING_CREDITS);
            order.setPayChannel(PayConstants.PAY_CHANNEL_SHOPPING_CREDITS);
            order.setUpdateTime(DateUtil.date());
            orderService.updateById(order);
            // 这里只扣除金额，账单记录在task中处理
            if (order.getPayPrice().compareTo(BigDecimal.ZERO) > 0) {
                update = userShoppingCreditsService.updateAmount(order.getUid(), merchantOrder.getMerId(), rechargeAmount, giftAmount, Constants.OPERATION_TYPE_SUBTRACT);
                if (!update) {
                    logger.error("购物金支付，扣除用户购物金失败，orderNo = {}", order.getOrderNo());
                    e.setRollbackOnly();
                    return update;
                }
                // 用户购物金记录
                ...
                record.setMerId(merchantOrder.getMerId());
                record.setLinkNo(order.getOrderNo());
                record.setLinkType("order");
                record.setType(BalanceRecordConstants.BALANCE_RECORD_TYPE_SUB);
                record.setTitle("购物金支付扣除");
        ...
        if (!execute) throw new CrmebException("购物金支付订单失败");
        asyncService.orderPaySuccessSplit(order.getOrderNo());
        return true;
    }
