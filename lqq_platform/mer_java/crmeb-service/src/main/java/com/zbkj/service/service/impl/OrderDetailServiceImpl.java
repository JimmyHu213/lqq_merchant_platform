...
public class OrderDetailServiceImpl extends ServiceImpl<OrderDetailDao, OrderDetail> implements OrderDetailService {
    ...
    public List<OrderDetail> getShipmentByOrderNo(String orderNo) {
        LambdaQueryWrapper<OrderDetail> lqw = Wrappers.lambdaQuery();
        lqw.select(OrderDetail::getId, OrderDetail::getProductName, OrderDetail::getProductType, OrderDetail::getImage, OrderDetail::getSku,
                OrderDetail::getPayNum, OrderDetail::getDeliveryNum, OrderDetail::getRefundNum, OrderDetail::getPrice,
                OrderDetail::getPayPrice, OrderDetail::getRefundPrice, OrderDetail::getApplyRefundNum, OrderDetail::getVerifyTotalTimes,
                OrderDetail::getVerifyRemainingTimes);
        lqw.eq(OrderDetail::getOrderNo, orderNo);
        return dao.selectList(lqw);
    }
    ...
    public PageInfo<OrderDetail> findPurchasedList(Integer userId, PageParamRequest pageParamRequest) {
        ...
        QueryWrapper<OrderDetail> query = Wrappers.query();
        query.select("ANY_VALUE(id) as id", "product_id");
        query.eq("uid", userId);
        query.gt("mer_id", 0);
        query.eq("is_receipt", 1);
        query.eq("product_marketing_type", 0);
        query.in("product_type", 0,2,5,6);
        query.groupBy("product_id");
        query.orderByDesc("id");
        List<OrderDetail> detailList = dao.selectList(query);
    ...
    public Integer getSalesNumCountByReservationIngo(Integer attrValueId, String reservationDate, String timeSlot) {
        return dao.getSalesNumCountByReservationIngo(attrValueId, reservationDate, timeSlot);
    }
     ...
     */
    @Override
    public BigDecimal getConsumptionAmountByMerAndUid(Integer merId, Integer userId) {
        return dao.getConsumptionAmountByMerAndUid(merId, userId);
    }
     ...
     */
    @Override
    public Integer getOrderProductNum(String date, Integer productId){
        return dao.getOrderProductNum(date, productId);
    }
     ...
     */
    @Override
    public Integer getSalesNumByDateAndMerId(String date, Integer merId) {
        return dao.getSalesNumByDateAndMerId(date, merId);
    }
}
