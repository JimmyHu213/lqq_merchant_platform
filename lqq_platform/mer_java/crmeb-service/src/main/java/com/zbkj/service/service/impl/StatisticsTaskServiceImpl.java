...
public class StatisticsTaskServiceImpl implements StatisticsTaskService {
    ...
    public void autoStatistics() {
       ...
       //  ========================================
        ...
        // 退款商品数
        Integer refundProductNum = refundOrderService.getRefundProductNumByDate(yesterdayStr);
        ...
        shoppingProductDayRecord.setRefundProductNum(refundProductNum);
        ...
        productDayRecordList.forEach(e -> {
            ...
            // 销量
            e.setOrderSuccessProductNum(orderDetailService.getSalesNumByDateAndProductId(yesterdayStr, e.getProductId()));
            // 销售额
            e.setOrderSuccessProductFee(orderDetailService.getSalesByDateAndProductId(yesterdayStr, e.getProductId()));
            // 下单总件数
            e.setOrderProductNum(orderDetailService.getOrderProductNum(yesterdayStr, e.getProductId()));
            // 退款件数
            e.setRefundProductNum(refundOrderInfoService.getRefundProductNum(yesterdayStr, e.getProductId()));
        });
        ...
        // 订单支付数量
        Integer payOrderNum = orderService.getPayOrderNumByDate(0, yesterdayStr);

        // 订单支付金额
        BigDecimal payOrderAmount = orderService.getPayOrderAmountByDate(0, yesterdayStr);

        // 订单退款数量
        Integer refundOrderNum = refundOrderService.getRefundOrderNumByDate(0, yesterdayStr);

        // 订单退款金额
        BigDecimal refundOrderAmount = refundOrderService.getRefundOrderAmountByDate(0, yesterdayStr);

        // 订单用劵金额
        BigDecimal couponOrderAmount = orderService.getCouponOrderAmountByDate(0, yesterdayStr);
        ...
        tradingDayRecord.setProductOrderCouponFee(couponOrderAmount);
