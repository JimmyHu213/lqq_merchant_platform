...
public class PayCallbackServiceImpl implements PayCallbackService {
    ...
    private String wechatCommonPayCallback(WxPayOrderNotifyResult wxPayOrderNotifyResult) {
        ...
        switch (attachVo.getType()) {
            ...
            case PayConstants.PAY_SERVICE_TYPE_SHOPPING_CREDITS:
                return wechatShoppingCreditsOrderCallback(wxPayOrderNotifyResult);
            default:
    ...
    private ResponseEntity<String> wechatCommonPayCallbackV3(WxPayNotifyV3Result wxPayNotifyV3Result) {
        ...
        switch (attachVo.getType()) {
            ...
            case PayConstants.PAY_SERVICE_TYPE_SVIP:
                ...
                break;
            case PayConstants.PAY_SERVICE_TYPE_SHOPPING_CREDITS:
                wechatShoppingCreditsOrderCallbackV3(wxPayNotifyV3Result);
                break;
            default:
                logger.error("wechat pay err : 未知的订单类型==》" + attachVo.getType());
                break;
    ...
    private String wechatShoppingCreditsOrderCallback(WxPayOrderNotifyResult wxPayOrderNotifyResult) {
        ShoppingCreditsOrder shoppingCreditsOrder = shoppingCreditsOrderService.getByOutTradeNo(wxPayOrderNotifyResult.getOutTradeNo());
        if (ObjectUtil.isNull(shoppingCreditsOrder)) {
            logger.error("wechat pay error : 购物金订单信息不存在==》" + wxPayOrderNotifyResult.getOutTradeNo());
            throw new CrmebException("wechat pay error : 购物金订单信息不存在==》" + wxPayOrderNotifyResult.getOutTradeNo());
        }
        ...
        if (ObjectUtil.isNull(wechatPayInfo)) {
            logger.error("wechat pay error : 微信购物金订单信息不存在==》" + wxPayOrderNotifyResult.getOutTradeNo());
            throw new CrmebException("wechat pay error : 微信购物金订单信息不存在==》" + wxPayOrderNotifyResult.getOutTradeNo());
        }
        wechatPayInfo.setIsSubscribe(wxPayOrderNotifyResult.getIsSubscribe());
        wechatPayInfo.setBankType(wxPayOrderNotifyResult.getBankType());
        wechatPayInfo.setCashFee(wxPayOrderNotifyResult.getCashFee());
        ...
        if (!shoppingCreditsPayAfter) {
            logger.error("wechat shopping credits pay after error : 购物金订单数据保存失败==》" + wxPayOrderNotifyResult.getOutTradeNo());
            throw new CrmebException("wechat shopping credits pay after error : 购物金订单数据保存失败==》" + wxPayOrderNotifyResult.getOutTradeNo());
        }
        return WxPayNotifyResponse.success("成功");
    }
...
//        wechatPayInfo.setTimeEnd(decryptRes.getTimeEnd());
    ...
    private void wechatShoppingCreditsOrderCallbackV3(WxPayNotifyV3Result wxPayNotifyV3Result) {
        WxPayNotifyV3Result.DecryptNotifyResult decryptRes = wxPayNotifyV3Result.getResult();
        String outTradeNo = decryptRes.getOutTradeNo();
        ShoppingCreditsOrder shoppingCreditsOrder = shoppingCreditsOrderService.getByOutTradeNo(outTradeNo);
        if (ObjectUtil.isNull(shoppingCreditsOrder)) {
            logger.error("wechat pay error : 购物金订单信息不存在==》" + outTradeNo);
            return;
        }
        if (shoppingCreditsOrder.getPaid()) {
            logger.info("wechat pay info : 购物金订单信息已支付==》" + outTradeNo);
            return;
        }
        WechatPayInfo wechatPayInfo = wechatPayInfoService.getByNo(shoppingCreditsOrder.getOutTradeNo());
        if (ObjectUtil.isNull(wechatPayInfo)) {
...
//        wechatPayInfo.setTimeEnd(decryptRes.getTimeEnd());
        wechatPayInfoService.updateById(wechatPayInfo);
        // 支付成功处理
        Boolean rechargePayAfter = shoppingCreditsOrderService.paySuccessAfter(shoppingCreditsOrder);
        if (!rechargePayAfter) {
            logger.error("wechat shopping credits pay after error : 购物金订单数据保存失败==》" + outTradeNo);
        }
    }
        ...
        try {
            ...
            if (StrUtil.isNotBlank(passbackParams)) {
                ...
                if (PayConstants.PAY_SERVICE_TYPE_SHOPPING_CREDITS.equals(orderType)) {// 购物金订单
                    ShoppingCreditsOrder shoppingCreditsOrder = shoppingCreditsOrderService.getByOutTradeNo(outTradeNo);
                    if (ObjectUtil.isNull(shoppingCreditsOrder)) {
                        logger.error("ali pay error : 购物金订单后置处理，没有找到对应订单，支付服务方订单号：{}", outTradeNo);
                        return "fail";
                    }
                    if (shoppingCreditsOrder.getPaid()) {
                        return "success";
                    ...
                    Boolean shoppingCreditsPayAfter = shoppingCreditsOrderService.paySuccessAfter(shoppingCreditsOrder);
                    if (!shoppingCreditsPayAfter) {
                        logger.error("ali pay shopping credits pay after error : 数据保存失败==》" + outTradeNo);
                        return "fail";
                    }
                    return "success";
                }
...
//            String aliPayPublicKey2 = systemConfigService.getValueByKey(AlipayConfig.ALIPAY_PUBLIC_KEY_2);
    ...
    private String wechatRefundOrderCallback(WxPayRefundNotifyResult wxPayRefundNotifyResult) {
        ...
        if (outRefundNo.contains(OrderConstants.ORDER_PREFIX_SHOPPING_CREDITS_REFUND)) {
            // 购物金退款
            return wechatShoppingCreditsRefundOrderCallback(wxPayRefundNotifyResult);
        }
    ...
    private String wechatShoppingCreditsRefundOrderCallback(WxPayRefundNotifyResult wxPayRefundNotifyResult) {
        String outRefundNo = wxPayRefundNotifyResult.getReqInfo().getOutRefundNo();
        ShoppingCreditsRefundOrder refundOrder = shoppingCreditsRefundOrderService.getByRefundOrderNo(outRefundNo);
        if (ObjectUtil.isNull(refundOrder)) {
            logger.error("微信购物金退款订单查询失败==>outRefundNo = {}, notifyResult = {}", outRefundNo, wxPayRefundNotifyResult);
            return WxPayNotifyResponse.fail("购物金退款订单查询失败");
        }
        if (refundOrder.getRefundStatus().equals(OrderConstants.MERCHANT_REFUND_ORDER_STATUS_REFUND)) {
            logger.error("微信购物金退款订单已确认成功==>outRefundNo = {}, notifyResult = {}", outRefundNo, wxPayRefundNotifyResult);
            return WxPayNotifyResponse.success("成功");
        }
        refundOrder.setRefundStatus(OrderConstants.MERCHANT_REFUND_ORDER_STATUS_REFUND);
        refundOrder.setUpdateTime(DateUtil.date());
        boolean update = shoppingCreditsRefundOrderService.updateById(refundOrder);
        if (!update) {
            logger.error("微信购物金退款订单更新失败==>outRefundNo = {}, notifyResult = {}", outRefundNo, wxPayRefundNotifyResult);
        }
        return WxPayNotifyResponse.success("成功");
    }
    ...
    private void wechatRefundOrderCallbackV3(WxPayRefundNotifyV3Result wxPayRefundNotifyV3Result) {
        ...
        if (outRefundNo.contains(OrderConstants.ORDER_PREFIX_SHOPPING_CREDITS_REFUND)) {
            wechatShoppingCreditsRefundOrderCallbackV3(wxPayRefundNotifyV3Result);
            return;
        }
        List<RefundOrder> refundOrderList = refundOrderService.findByOutRefundNo(outRefundNo);
        if (CollUtil.isEmpty(refundOrderList)) {
    ...
    private void wechatShoppingCreditsRefundOrderCallbackV3(WxPayRefundNotifyV3Result wxPayRefundNotifyV3Result) {
        WxPayRefundNotifyV3Result.DecryptNotifyResult decryptRes = wxPayRefundNotifyV3Result.getResult();
        String outRefundNo = decryptRes.getOutRefundNo();
        ShoppingCreditsRefundOrder refundOrder = shoppingCreditsRefundOrderService.getByRefundOrderNo(outRefundNo);
        if (ObjectUtil.isNull(refundOrder)) {
            logger.error("微信购物金退款订单查询失败==>outRefundNo = {}, notifyResult = {}", outRefundNo, decryptRes);
            return;
        }
        if (refundOrder.getRefundStatus().equals(OrderConstants.MERCHANT_REFUND_ORDER_STATUS_REFUND)) {
            logger.error("微信购物金退款订单已确认成功==>outRefundNo = {}, notifyResult = {}", outRefundNo, decryptRes);
            return;
        }
        refundOrder.setRefundStatus(OrderConstants.MERCHANT_REFUND_ORDER_STATUS_REFUND);
        refundOrder.setUpdateTime(DateUtil.date());
        boolean update = shoppingCreditsRefundOrderService.updateById(refundOrder);
        if (!update) {
            logger.error("微信购物金退款订单更新失败==>outRefundNo = {}, notifyResult = {}", outRefundNo, decryptRes);
        }
    }
