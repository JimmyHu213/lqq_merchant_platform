...
public class CouponUserServiceImpl extends ServiceImpl<CouponUserDao, CouponUser> implements CouponUserService {
    ...
    public Boolean receiveCoupon(Integer cid) {
        ...
        // 商户会员优惠券校验
        if (coupon.getIdentityType().equals(1)) {
            MerchantMember merchantMember = merchantMemberService.getByUidAndMerId(userId, coupon.getMerId());
            if (ObjectUtil.isNull(merchantMember) || merchantMember.getIsLogoff()) {
                throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "商户会员才可领取此优惠券！");
            }
        }
        //看是否有剩余数量,是否够给当前用户
    ...
    @Override
    public Boolean userIsCanReceiveCoupon(Coupon coupon, Integer userId, Boolean isMerchantMember) {
        if (coupon.getIdentityType().equals(1) && !isMerchantMember) {
            return false;
        }
        LambdaQueryWrapper<CouponUser> lqw = new LambdaQueryWrapper<>();
        lqw.eq(CouponUser::getCouponId, coupon.getId());
     ...
     */
    @Override
    public void batchReceiveCoupon(List<Coupon> couponList, Integer userId) {
