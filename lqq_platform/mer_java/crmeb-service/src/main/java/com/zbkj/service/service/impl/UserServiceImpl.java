...
//        SystemAdmin systemAdmin = SecurityUtil.getLoginUserVo().getUser();
        Page<User> pageUser = PageHelper.startPage(request.getPage(), request.getLimit());
//        Map<String, Object> map = CollUtil.newHashMap();
//        // if (StrUtil.isNotBlank(request.getUserType()) && request.getUserType().equals("collect")) {
...
//        if (StrUtil.isNotBlank(request.getUserType()) && request.getUserType().equals("all")) {
//            userList = dao.findUserAllList(map);
//        } else {
//            userList = dao.findMerchantList(map);
//        }
...
//        }
    ...
    public Boolean logoff() {
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            merchantMemberService.logoffByUserId(user.getId());
            return Boolean.TRUE;
        });
        if (!execute) {
...
//        lqw.eq(User::getBirthday, birthday);
    ...
    public List<User> findRetailStorePeopleList(RetailStorePeopleSearchRequest request) {
        ...
        return dao.findRetailStorePeopleList(map);
    }
     ...
     */
    @Override
    public Map<String, Integer> getFixedUserData() {
        Map<String, Integer> map = new HashMap<>();
        // 用户数量
        QueryWrapper<User> lqw = Wrappers.query();
        Integer userNum = dao.selectCount(lqw);
        // 付费会员数量
        lqw = Wrappers.query();
        lqw.eq("is_paid_member", true);
        Integer paidMemberNum = dao.selectCount(lqw);
        map.put("userNum", userNum);
        map.put("paidMemberNum", paidMemberNum);
        return map;
    }

    /**
     * 获取新增用户数量
     * @param dateRequest 日期
     */
    @Override
    public Integer getAddUserNum(DateRequest dateRequest) {
        QueryWrapper<User> lqw = Wrappers.query();
     ...
     */
    @Override
    public Integer getUserAddNum(String date, DateRequest dateRequest) {
        QueryWrapper<User> lqw = Wrappers.query();
     ...
     */
    @Override
    public Integer getUserAddNumByUidList(String date, List<Integer> uidList) {
        if (CollUtil.isEmpty(uidList)) {
            return 0;
        }
        QueryWrapper<User> lqw = Wrappers.query();
