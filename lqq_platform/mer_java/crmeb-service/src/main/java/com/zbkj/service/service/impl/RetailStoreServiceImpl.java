...
import com.zbkj.common.vo.RetailStoreBaseConfigVo;
import com.zbkj.common.vo.RetailStoreConfigVo;
import com.zbkj.common.vo.RetailStoreExtractConfigVo;
...
public class RetailStoreServiceImpl implements RetailStoreService {
     ...
     */
    @Override
    public RetailStoreBaseConfigVo getRetailStoreBaseConfig() {
        ArrayList<String> keyList = new ArrayList<>();
        keyList.add(SysConfigConstants.RETAIL_STORE_SWITCH);
        keyList.add(SysConfigConstants.RETAIL_STORE_LINE);
        keyList.add(SysConfigConstants.RETAIL_STORE_BINDING_TYPE);
        ...
        RetailStoreBaseConfigVo vo = new RetailStoreBaseConfigVo();
        vo.setRetailStoreSwitch(Integer.parseInt(record.getStr(SysConfigConstants.RETAIL_STORE_SWITCH)));
        vo.setRetailStoreLine(Integer.parseInt(record.getStr(SysConfigConstants.RETAIL_STORE_LINE)));
        vo.setRetailStoreBindingType(Integer.parseInt(record.getStr(SysConfigConstants.RETAIL_STORE_BINDING_TYPE)));
        vo.setRetailStoreBrokerageFirstRatio(Integer.parseInt(record.getStr(SysConfigConstants.RETAIL_STORE_BROKERAGE_FIRST_RATIO)));
     ...
     */
    @Override
    public Boolean saveRetailStoreBaseConfig(RetailStoreBaseConfigVo baseConfigVo) {
        // 返佣比例之和+起来不能超过100%
        int ration = baseConfigVo.getRetailStoreBrokerageFirstRatio() + baseConfigVo.getRetailStoreBrokerageSecondRatio();
        if (ration > crmebConfig.getRetailStoreBrokerageRatio() || ration < 0) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, StrUtil.format("返佣比例之和范围为 0 ~ {}", crmebConfig.getRetailStoreBrokerageRatio()));
        }

        systemConfigService.updateOrSaveValueByName(SysConfigConstants.RETAIL_STORE_SWITCH, baseConfigVo.getRetailStoreSwitch().toString());
        systemConfigService.updateOrSaveValueByName(SysConfigConstants.RETAIL_STORE_LINE, baseConfigVo.getRetailStoreLine().toString());
        systemConfigService.updateOrSaveValueByName(SysConfigConstants.RETAIL_STORE_BINDING_TYPE, baseConfigVo.getRetailStoreBindingType().toString());
        systemConfigService.updateOrSaveValueByName(SysConfigConstants.RETAIL_STORE_BROKERAGE_FIRST_RATIO, baseConfigVo.getRetailStoreBrokerageFirstRatio().toString());
        systemConfigService.updateOrSaveValueByName(SysConfigConstants.RETAIL_STORE_BROKERAGE_SECOND_RATIO, baseConfigVo.getRetailStoreBrokerageSecondRatio().toString());
        systemConfigService.updateOrSaveValueByName(SysConfigConstants.RETAIL_STORE_BROKERAGE_FREEZING_TIME, baseConfigVo.getRetailStoreBrokerageFreezingTime().toString());
        systemConfigService.updateOrSaveValueByName(SysConfigConstants.RETAIL_STORE_BROKERAGE_SHARE_NODE, baseConfigVo.getRetailStoreBrokerageShareNode());
        return true;
    }
     ...
     */
    @Override
    public RetailStoreExtractConfigVo getRetailStoreExtractConfig() {
        ArrayList<String> keyList = new ArrayList<>();
        keyList.add(SysConfigConstants.RETAIL_STORE_EXTRACT_METHOD);
        keyList.add(SysConfigConstants.RETAIL_STORE_EXTRACT_WECHAT_METHOD);
        keyList.add(SysConfigConstants.RETAIL_STORE_EXTRACT_WECHAT_SCENE_ID);
        keyList.add(SysConfigConstants.RETAIL_STORE_EXTRACT_MIN_PRICE);
        keyList.add(SysConfigConstants.RETAIL_STORE_EXTRACT_BANK);
        MyRecord record = systemConfigService.getValuesByKeyList(keyList);

        RetailStoreExtractConfigVo vo = new RetailStoreExtractConfigVo();
        vo.setRetailStoreExtractMethod(record.getStr(SysConfigConstants.RETAIL_STORE_EXTRACT_METHOD));
        vo.setRetailStoreExtractWechatMethod(Integer.parseInt(record.getStr(SysConfigConstants.RETAIL_STORE_EXTRACT_WECHAT_METHOD)));
        vo.setRetailStoreExtractWechatSceneId(record.getStr(SysConfigConstants.RETAIL_STORE_EXTRACT_WECHAT_SCENE_ID));
        vo.setRetailStoreExtractMinPrice(new BigDecimal(record.getStr(SysConfigConstants.RETAIL_STORE_EXTRACT_MIN_PRICE)));
        vo.setRetailStoreExtractBank(record.getStr(SysConfigConstants.RETAIL_STORE_EXTRACT_BANK).replace("\\n", "\n"));
        return vo;
    }
     ...
     */
    @Override
    public Boolean saveRetailStoreExtractConfig(RetailStoreExtractConfigVo extractConfigVo) {
        String retailStoreExtractMethod = StrUtil.isBlank(extractConfigVo.getRetailStoreExtractMethod()) ? "" : extractConfigVo.getRetailStoreExtractMethod();
        systemConfigService.updateOrSaveValueByName(SysConfigConstants.RETAIL_STORE_EXTRACT_METHOD, retailStoreExtractMethod);
        systemConfigService.updateOrSaveValueByName(SysConfigConstants.RETAIL_STORE_EXTRACT_WECHAT_METHOD, extractConfigVo.getRetailStoreExtractWechatMethod().toString());
        systemConfigService.updateOrSaveValueByName(SysConfigConstants.RETAIL_STORE_EXTRACT_WECHAT_SCENE_ID, extractConfigVo.getRetailStoreExtractWechatSceneId());
        systemConfigService.updateOrSaveValueByName(SysConfigConstants.RETAIL_STORE_EXTRACT_MIN_PRICE, extractConfigVo.getRetailStoreExtractMinPrice().toString());
        systemConfigService.updateOrSaveValueByName(SysConfigConstants.RETAIL_STORE_EXTRACT_BANK, extractConfigVo.getRetailStoreExtractBank());
        return true;
    }
    ...
    public UserClosingConfigResponse getUserClosingConfig() {
        ...
        // 获取提现方式 wechat,bankCard,alipay,balance
        String extractMethod = systemConfigService.getValueByKeyException(SysConfigConstants.RETAIL_STORE_EXTRACT_METHOD);
        // 获取提现银行
        String bank = "";
        if (StrUtil.isNotBlank(extractMethod) && extractMethod.contains("bankCard")) {
            String extractBank = systemConfigService.getValueByKey(SysConfigConstants.RETAIL_STORE_EXTRACT_BANK);
            if (StrUtil.isNotBlank(extractBank)) {
                bank = extractBank;
            }
        }
        // 获取微信到账方式
        String extractWechatMethod = systemConfigService.getValueByKeyException(SysConfigConstants.RETAIL_STORE_EXTRACT_WECHAT_METHOD);
        ...
        response.setExtractMethod(extractMethod);
        response.setExtractWechatMethod(extractWechatMethod);
        return response;
    }
     ...
     */
    @Override
    public UserClosingApplyResponse userClosingApply(UserClosingApplyRequest request) {
        if (request.getType().equals(ClosingConstant.CLOSING_TYPE_WECHAT)) {
            String wechatMethod = systemConfigService.getValueByKey(SysConfigConstants.RETAIL_STORE_EXTRACT_WECHAT_METHOD);
            if (wechatMethod.equals("1")) {
                // 微信佣金提现到零钱
                return wechatExtract(request.getClosingPrice());
            }
        }
        ...
        // 此处可添加提现申请通知
        UserClosingApplyResponse response = new UserClosingApplyResponse();
        response.setApplyResult(execute);
        return response;
    }
     ...
     * 微信提现到零钱
     *
     * @param closingPrice 结算金额
     */
    private UserClosingApplyResponse wechatExtract(BigDecimal closingPrice) {
        String minPrice = systemConfigService.getValueByKeyException(SysConfigConstants.RETAIL_STORE_EXTRACT_MIN_PRICE);
        if (closingPrice.compareTo(new BigDecimal(minPrice)) < 0) {
        ...
        String wechatOpenId = userTokenService.getWechatOpenId(user.getId());
        if (StrUtil.isBlank(wechatOpenId)) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "未查询到用户微信侧openid");
        }
        ...
        if (money.compareTo(closingPrice) < 0) {
            throw new CrmebException(UserResultCode.USER_BROKERAGE_INSUFFICIENT.setMessage("你当前最多可提现 " + money + "元"));
        }
        UserClosing userClosing = new UserClosing();
        userClosing.setClosingType(ClosingConstant.CLOSING_TYPE_WECHAT);
        userClosing.setClosingPrice(closingPrice);
        userClosing.setUid(user.getId());
        userClosing.setBalance(money.subtract(closingPrice));
        userClosing.setClosingNo(CrmebUtil.getOrderNo(OrderConstants.CLOSING_ORDER_PREFIX_USER));
        //存入银行名称
        if (StrUtil.isNotBlank(userClosing.getPaymentCode())) {
            userClosing.setPaymentCode(systemAttachmentService.clearPrefix(userClosing.getPaymentCode()));
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            // 保存提现记录
            result = userClosingService.save(userClosing);
            if (!result) {
                e.setRollbackOnly();
                return Boolean.FALSE;
            }
            // 添加佣金记录
            result = userBrokerageRecordService.save(brokerageRecord);
            if (!result) {
                e.setRollbackOnly();
                return Boolean.FALSE;
            }
        ...
        if (execute) {
            // 微信商家转账到零钱
            MyRecord record = wechatPayService.transferToSmallChange(wechatOpenId, userClosing.getClosingNo(), userClosing.getClosingPrice());
            response.setMchId(record.getStr("mchId"));
            response.setAppId(record.getStr("appId"));
            response.setPackageInfo(record.getStr("packageInfo"));
            response.setOrderNo(userClosing.getClosingNo());
        }
        return response;
    }
