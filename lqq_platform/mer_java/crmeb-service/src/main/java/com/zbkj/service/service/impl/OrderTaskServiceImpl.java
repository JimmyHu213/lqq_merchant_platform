...
public class OrderTaskServiceImpl implements OrderTaskService {
    ...
    private Boolean refundAfterProcessing(String refundOrderNo) {
        ...
        // 次卡订单退款逻辑
        Boolean isVerify = Boolean.FALSE;  // 次卡订单是否核销过
        if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CARDTIMES)) {
            if (orderDetail.getVerifyRemainingTimes() < orderDetail.getVerifyTotalTimes()) {
                isVerify = Boolean.TRUE; // 已核销过或部分核销
            }
        }
        Boolean finalIsVerify = isVerify;
        return transactionTemplate.execute(e -> {
            ...
            // 次卡订单核销过，优惠卷不退回
            if (CollUtil.isNotEmpty(couponIdList) && !finalIsVerify) {
    ...
    private void refundRollbackStock(RefundOrderInfo refundOrderInfo, Integer orderType, Integer orderSecondType) {
        ...
        if (orderSecondType.equals(OrderConstants.ORDER_SECOND_TYPE_RESERVATION)) {
            Product product = productService.getById(refundOrderInfo.getProductId());
            productService.operationSales(product.getId(), refundOrderInfo.getApplyRefundNum(), Constants.OPERATION_TYPE_SUBTRACT);
            ProductAttrValue productAttrValue = productAttrValueService.getById(refundOrderInfo.getAttrValueId());
            productAttrValueService.operationSales(productAttrValue.getId(), refundOrderInfo.getApplyRefundNum(), Constants.OPERATION_TYPE_SUBTRACT);
            return;
        }
    ...
    private Boolean commonOrderCancel(Order order, String cancelMethod) {
        ...
        return transactionTemplate.execute(e -> {
            if (cancelMethod.equals("auto")) {
                ...
                orderFlowRecordService.orderCancel(order.getOrderNo(), 0, cancelMethod);
            }
            // 退优惠券
    ...
    private Boolean rollbackStock(Order order) {
        ...
        if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_RESERVATION)) {
            ...
            if (ObjectUtil.isNotNull(product)) {
                result = productService.operationSales(product.getId(), orderDetail.getPayNum(), Constants.OPERATION_TYPE_SUBTRACT);
                if (!result) {
                    logger.error("订单回滚销量——商品处理失败,订单号:{}", order.getOrderNo());
                    return result;
                }
            }
            ...
            if (ObjectUtil.isNotNull(productAttrValue)) {
                result = productAttrValueService.operationSales(productAttrValue.getId(), orderDetail.getPayNum(),
                        Constants.OPERATION_TYPE_SUBTRACT);
                if (!result) {
                    logger.error("订单回滚销量——商品sku处理失败,订单号:{}", order.getOrderNo());
                    return result;
                }
            }
            return Boolean.TRUE;
        ...
        for (OrderDetail orderDetail : orderDetailList) {
            if (order.getType().equals(OrderConstants.ORDER_TYPE_PITUAN)) {
                // 拼团订单返回库存处理
                GroupBuyActivitySku groupBuyActivitySku = groupBuyActivitySkuService.getByAttrId(orderDetail.getGroupBuyActivityId(),orderDetail.getAttrValueId());
                if (ObjectUtil.isNotNull(groupBuyActivitySku)) {
                    Boolean rollBackSKUStock = groupBuyActivitySkuService.rollBackSKUStock(order, groupBuyActivitySku.getGroupActivityId(),
                            groupBuyActivitySku.getSkuId(), orderDetail.getPayNum());
                    if (!rollBackSKUStock) {
    ...
    public void autoTakeDelivery() {
        ...
        orderList = orderList.stream().filter(order -> !order.getSecondType().equals(4)).collect(Collectors.toList());
        if (CollUtil.isEmpty(orderList)) {
            return;
        }
        List<String> orderNoList = orderList.stream().map(Order::getOrderNo).collect(Collectors.toList());
        Boolean execute = transactionTemplate.execute(e -> {
            orderNoList.forEach(orderNo -> {
                ...
                orderFlowRecordService.orderReceipt(orderNo, 0, "auto", null);
            });
            return Boolean.TRUE;
        });
        if (!execute) {
