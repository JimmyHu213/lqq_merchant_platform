...
import com.zbkj.common.request.MerchantStreetRequest;
...
public class MerchantServiceImpl extends ServiceImpl<MerchantDao, Merchant> implements MerchantService {
    ...
    public Boolean add(MerchantAddRequest request) {
        ...
        if (execute) {
            ...
            if (StrUtil.isNotBlank(request.getPhone()) && payNotification.getIsSms().equals(1)) {
                String siteName = systemConfigService.getValueByKey(SysConfigConstants.CONFIG_KEY_SITE_NAME);
    ...
    private Boolean commonAdd(MerchantAddRequest request, String createType, Integer createId) {
        ...
        groupConfigService.saveBatch(groupConfigList);
        return save;
    }
...
//            wrapper.last(StrUtil.format("balance - {} >= 0", price));
    ...
    public List<Merchant> getListByIdList(List<Integer> merIdList) {
        ...
        lqw.eq(Merchant::getIsDel, Boolean.FALSE);
    ...
    public PageInfo<MerchantSettledResponse> findSettledRecord(PageParamRequest pageParamRequest) {
        ...
        List<MerchantSettledResponse> responseList = merchantApplyList.stream().map(apply -> {
            ...
            if (response.getAuditStatus().equals(MerchantConstants.AUDIT_STATUS_SUCCESS)) {
                response.setMerSiteUrl(systemConfigService.getValueByKey(SysConfigConstants.CONFIG_KEY_MERCHANT_SITE_URL));
            }
            return response;
        }).collect(Collectors.toList());
     ...
     */
    @Override
    public PageInfo<MerchantSearchResponse> getStreet(MerchantStreetRequest request) {
        ...
        Page<Merchant> page = PageHelper.startPage(request.getPage(), request.getLimit());
        List<Merchant> merchantList = dao.selectList(lqw);
        if (CollUtil.isEmpty(merchantList)) {
            return CommonPage.copyPageInfo(page, CollUtil.newArrayList());
        }
    ...
    public List<CategoryMerchantResponse> getUseCategoryList() {
        LambdaQueryWrapper<Merchant> lqw = Wrappers.lambdaQuery();
        lqw.select(Merchant::getId, Merchant::getName, Merchant::getCategoryId, Merchant::getRealName, Merchant::getPhone);
        lqw.eq(Merchant::getIsSwitch, 1);
        lqw.eq(Merchant::getIsDel, 0);
        List<Merchant> merchantList = dao.selectList(lqw);
     ...
     */
    @Override
    public Merchant getLocationInfo(Integer merId) {
        LambdaQueryWrapper<Merchant> lqw = Wrappers.lambdaQuery();
        lqw.select(Merchant::getId, Merchant::getName, Merchant::getAddressDetail, Merchant::getCoverImage, Merchant::getLatitude, Merchant::getLongitude);
        lqw.eq(Merchant::getId, merId);
        lqw.last(" limit 1");
        return dao.selectOne(lqw);
    }

    @Override
    public PageInfo<MerchantPageResponse> getCircleMerchantPage(List<Integer> merIdList, MerchantSearchRequest searchRequest, PageParamRequest pageParamRequest) {
        Page<Merchant> page = PageHelper.startPage(pageParamRequest.getPage(), pageParamRequest.getLimit());
        LambdaQueryWrapper<Merchant> lqw = getAdminPageWrapperBySearch(searchRequest);
        lqw.in(Merchant::getId, merIdList);
        lqw.orderByDesc(Merchant::getSort);
     ...
     */
    @Override
    public MerchantHeaderNumResponse getCircleMerchantNum(List<Integer> merIdList, MerchantSearchRequest searchRequest) {
        LambdaQueryWrapper<Merchant> lqw = getAdminPageWrapperBySearch(searchRequest);
        lqw.eq(Merchant::getIsSwitch, 1);
        lqw.in(Merchant::getId, merIdList);
        Integer openNum = dao.selectCount(lqw);
        LambdaQueryWrapper<Merchant> wrapper = getAdminPageWrapperBySearch(searchRequest);
        wrapper.eq(Merchant::getIsSwitch, 0);
        wrapper.in(Merchant::getId, merIdList);
        Integer closeNum = dao.selectCount(wrapper);
        return new MerchantHeaderNumResponse(openNum, closeNum);
    }
     ...
     */
    @Override
    public List<Merchant> getRegionStreet(List<Integer> merIdList) {
        LambdaQueryWrapper<Merchant> lqw = Wrappers.lambdaQuery();
        if (CollUtil.isNotEmpty(merIdList)) {
            lqw.in(Merchant::getId, merIdList);
        }
     ...
     */
    @Override
    public List<Merchant> findRegionHomeMerchantList(List<Integer> merIdList, Integer num) {
        LambdaQueryWrapper<Merchant> lqw = Wrappers.lambdaQuery();
        lqw.eq(Merchant::getIsSwitch, true);
        lqw.eq(Merchant::getIsDel, false);
        if (CollUtil.isNotEmpty(merIdList)) {
     ...
     */
    @Override
    public List<CircleMerchantResponse> getCircleMerchantsUnBind(Integer circleId) {
        return dao.selectCircleMerchantsUnBind(circleId);
    }
     ...
     */
    @Override
    public List<Merchant> findIndexMerchantList(List<Integer> merIdList, Integer num) {
        LambdaQueryWrapper<Merchant> lqw = Wrappers.lambdaQuery();
        lqw.eq(Merchant::getIsSwitch, true);
        lqw.eq(Merchant::getIsDel, false);
        if (CollUtil.isNotEmpty(merIdList)) {
