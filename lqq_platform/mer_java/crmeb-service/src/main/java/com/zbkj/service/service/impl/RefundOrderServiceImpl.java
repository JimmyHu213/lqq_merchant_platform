...
public class RefundOrderServiceImpl extends ServiceImpl<RefundOrderDao, RefundOrder> implements RefundOrderService {
    ...
    private void settingOrderStatus(Order order) {
        ...
        if (refundCount == orderDetailList.size()) {
            ...
            orderFlowRecordService.orderRefund(order.getOrderNo());
            return;
        }
    ...
    public PageInfo<RefundOrderResponse> getH5List(OrderAfterSalesSearchRequest request) {
        ...
        Page<RefundOrderResponse> page = PageHelper.startPage(request.getPage(), request.getLimit());
    ...
    public RefundOrderAdminDetailResponse getMerchantDetail(String refundOrderNo, SystemAdmin systemAdmin) {
        ...
        if (orderInfoVo.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CARDTIMES)) {
            List<OrderDetail> orderDetailList = orderDetailService.getByOrderNo(refundOrder.getOrderNo());
            ...
            response.setVerifyStartDate(orderDetail.getVerifyStartDate());
            response.setVerifyEndDate(orderDetail.getVerifyEndDate());
        }
        return response;
    }
    ...
    private RefundOrderDetailOrderInfoVo getRefundOrderDetailOrderInfo(String orderNo) {
        ...
        orderInfoVo.setMerchantMemberDiscountPrice(order.getMerchantMemberDiscountPrice());
    ...
    public Boolean merchantDirectRefund(Order order, List<OrderDetail> orderDetailList, List<Integer> workOrderIdList) {
        ...
        for (OrderDetail orderDetail : orderDetailList) {
            ...
            BigDecimal refundRatio = new BigDecimal(1);  // 次卡退款比例
            if (orderDetail.getPayNum().equals(refundOrderInfo.getApplyRefundNum())) {
                ...
                if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CARDTIMES)) {
                    refundRatio = BigDecimal.valueOf(orderDetail.getVerifyRemainingTimes())
                            .divide(BigDecimal.valueOf(orderDetail.getVerifyTotalTimes()), 10, RoundingMode.HALF_UP);
                    refundPrice = refundRatio.multiply(orderDetail.getPayPrice())
                            .setScale(2, RoundingMode.DOWN);
                }
                orderDetail.setRefundPrice(refundPrice);
                refundOrderInfo.setRefundPrice(refundPrice);

                Integer useIntegral = orderDetail.getUseIntegral();
                BigDecimal integralPrice = orderDetail.getIntegralPrice();
                if (useIntegral > 0) {
                    if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CARDTIMES)) {
                        useIntegral = refundRatio.multiply(new BigDecimal(useIntegral)).setScale(0, RoundingMode.DOWN).intValue();
                        integralPrice = integralPrice.multiply(refundRatio).setScale(2, RoundingMode.DOWN);
                    }
                    orderDetail.setRefundUseIntegral(useIntegral);
                    orderDetail.setRefundIntegralPrice(integralPrice);
                ...
                Integer gainIntegral = orderDetail.getGainIntegral();
                if (gainIntegral > 0) {
                    if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CARDTIMES)) {
                        gainIntegral = refundRatio.multiply(new BigDecimal(gainIntegral)).setScale(0, RoundingMode.UP).intValue();
                    }
                    orderDetail.setRefundGainIntegral(gainIntegral);
                    refundOrderInfo.setRefundGainIntegral(orderDetail.getRefundGainIntegral());
                }
                BigDecimal firstBrokerageFee = orderDetail.getFirstBrokerageFee();
                if (firstBrokerageFee.compareTo(BigDecimal.ZERO) > 0) {
                    if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CARDTIMES)) {
                        firstBrokerageFee = firstBrokerageFee.multiply(refundRatio).setScale(2, RoundingMode.UP);
                    }
                    orderDetail.setRefundFirstBrokerageFee(firstBrokerageFee);
                    refundOrderInfo.setRefundFirstBrokerageFee(orderDetail.getRefundFirstBrokerageFee());
                }
                BigDecimal secondBrokerageFee = orderDetail.getSecondBrokerageFee();
                if (secondBrokerageFee.compareTo(BigDecimal.ZERO) > 0) {
                    if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CARDTIMES)) {
                        secondBrokerageFee = secondBrokerageFee.multiply(refundRatio).setScale(2, RoundingMode.UP);
                    }
                    orderDetail.setRefundSecondBrokerageFee(secondBrokerageFee);
        ...
        List<UserShoppingCreditsRecord> creditsRecordList = new ArrayList<>();
        if (order.getPayType().equals(PayConstants.PAY_TYPE_SHOPPING_CREDITS)) {
            for (RefundOrder refundOrder : refundOrderList) {
                if (refundOrder.getRefundPrice().compareTo(BigDecimal.ZERO) > 0) {
                    UserShoppingCreditsRecord creditsRefundRecord = initUserShoppingCreditsRefundRecord(order.getUid(),
                            order.getMerId(), order.getOrderNo(), refundOrder.getRefundOrderNo(),
                            refundOrder.getRefundPrice(), order.getPlatOrderNo());
                    creditsRecordList.add(creditsRefundRecord);
                }
            }
        }
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            if (order.getPayType().equals(PayConstants.PAY_TYPE_SHOPPING_CREDITS)) {
                ...
                if (CollUtil.isNotEmpty(creditsRecordList)) {
                    for (UserShoppingCreditsRecord creditsRefundRecord : creditsRecordList) {
                        // 更新用户购物金
                        userShoppingCreditsService.updateAmount(order.getUid(), order.getMerId(),
                                creditsRefundRecord.getRechargeAmount(), creditsRefundRecord.getGiftAmount(),
                                Constants.OPERATION_TYPE_ADD);
                        // 查询订单对应 用户购物金记录
                        userShoppingCreditsRecordService.save(creditsRefundRecord);
                    }
                }
            }
            ...
            if (CollUtil.isNotEmpty(workOrderIdList)) {
                reservationWorkOrderService.batchRefundByIdList(workOrderIdList);
            }
            return Boolean.TRUE;
        });
        if (execute) {
    ...
    public PageInfo<PlatformRefundOrderPageResponse> getCircleRefundOrderPage(RefundOrderSearchRequest request, List<Integer> merIdList) {
        ...
        List<PlatformRefundOrderPageResponse> responseList = dao.getCircleRefundOrderPage(map);
        return CommonPage.copyPageInfo(page, responseList);
    }
    ...
    public RefundOrderCountItemResponse getCircleRefundOrderStatusNum(RefundOrderSearchRequest request, List<Integer> merIdList) {
        ...
        // 全部订单
        response.setAll(getCircleRefundOrderCount(request, 9, merIds));
        // 待审核
        response.setAwait(getCircleRefundOrderCount(request, OrderConstants.MERCHANT_REFUND_ORDER_STATUS_APPLY, merIds));
        // 审核拒绝
        response.setReject(getCircleRefundOrderCount(request, OrderConstants.MERCHANT_REFUND_ORDER_STATUS_REJECT, merIds));
        // 退款中
        response.setRefunding(getCircleRefundOrderCount(request, OrderConstants.MERCHANT_REFUND_ORDER_STATUS_REFUNDING, merIds));
        // 已退款
        response.setRefunded(getCircleRefundOrderCount(request, OrderConstants.MERCHANT_REFUND_ORDER_STATUS_REFUND, merIds));
        response.setAwaitReturning(getCircleRefundOrderCount(request, OrderConstants.MERCHANT_REFUND_ORDER_STATUS_RETURN_GOODS, merIds));
        response.setAwaitReceiving(getCircleRefundOrderCount(request, OrderConstants.MERCHANT_REFUND_ORDER_STATUS_AWAIT_RECEIVING, merIds));
        response.setRevoke(getCircleRefundOrderCount(request, OrderConstants.MERCHANT_REFUND_ORDER_STATUS_REVOKE, merIds));
        return response;
    }
     ...
     */
    @Override
    public Integer getRefundProductNumByDate(String date) {
        QueryWrapper<RefundOrder> wrapper = new QueryWrapper<>();
        wrapper.select("IFNULL(sum(total_num), 0) as total_num");
        wrapper.eq("refund_status", OrderConstants.MERCHANT_REFUND_ORDER_STATUS_REFUND);
        wrapper.apply("date_format(refund_time, '%Y-%m-%d') = {0}", date);
        RefundOrder refundOrder = dao.selectOne(wrapper);
        return refundOrder.getTotalNum();
    }
    ...
    private Boolean refundPrice(RefundOrder refundOrder, Order order) {
        ...
        if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_RESERVATION)) {
            Integer workOrderCount = reservationWorkOrderService.getCountByOrderNo(order.getOrderNo());
            if (workOrderCount > 0) {
                List<ReservationWorkOrder> workOrderList = findAwaitRefundWorkOrderList(orderDetail.getPayNum(), orderDetail.getRefundNum(), refundOrderInfo.getApplyRefundNum(), order.getOrderNo());
        ...
        UserShoppingCreditsRecord creditsRefundRecord;
        if (order.getPayType().equals(PayConstants.PAY_TYPE_SHOPPING_CREDITS) && refundOrder.getRefundPrice().compareTo(BigDecimal.ZERO) > 0) {
            creditsRefundRecord = initUserShoppingCreditsRefundRecord(order.getUid(), order.getMerId(), order.getOrderNo(),
                    refundOrder.getRefundOrderNo(), refundOrder.getRefundPrice(), order.getPlatOrderNo());
        } else {
            creditsRefundRecord = null;
        }
        Boolean execute = transactionTemplate.execute(e -> {
            orderDetailService.updateById(orderDetail);
        ...
        if (execute) {
            ...
            orderService.updateById(order);
            // 积分、佣金、优惠券等放入后置task中处理
            if (order.getPayType().equals(PayConstants.PAY_TYPE_YUE) || order.getPayType().equals(PayConstants.PAY_TYPE_SHOPPING_CREDITS)) {
                redisUtil.lPush(TaskConstants.ORDER_TASK_REDIS_KEY_AFTER_REFUND_BY_USER, refundOrder.getRefundOrderNo());
            }
            if (order.getPayType().equals(PayConstants.PAY_TYPE_ALI_PAY) && refundPrice.compareTo(BigDecimal.ZERO) > 0) {
                if (aliPayIsRefund) {
    ...
    private UserShoppingCreditsRecord initUserShoppingCreditsRefundRecord(Integer userId, Integer merId, String orderNo, String refundOrderNo, BigDecimal refundPrice, String platOrderNo) {
        List<RefundOrder> refundOrderList = findByOrderNo(orderNo);

        UserShoppingCreditsRecord orderRecord = userShoppingCreditsRecordService.getByLinkNoAndType(platOrderNo, "order");
        BigDecimal rechargeRefundAmount;
        BigDecimal giftRefundAmount;
        if (CollUtil.isEmpty(refundOrderList)) {// 首单退款
            if (orderRecord.getRechargeAmount().compareTo(BigDecimal.ZERO) == 0) {
                // 仅使用赠送购物金
                rechargeRefundAmount = new BigDecimal("0.00");
                giftRefundAmount = refundPrice;
            } else if (orderRecord.getGiftAmount().compareTo(BigDecimal.ZERO) == 0) {
                // 仅使用充值购物金
                rechargeRefundAmount = refundPrice;
                giftRefundAmount = new BigDecimal("0.00");
            } else {
                // 同时使用充值 + 赠送，先退赠送部分
                if (orderRecord.getGiftAmount().compareTo(refundPrice) >= 0) {
                    rechargeRefundAmount = new BigDecimal("0.00");
                    giftRefundAmount = refundPrice;
                } else {
                    rechargeRefundAmount = refundPrice.subtract(orderRecord.getGiftAmount());
                    giftRefundAmount = orderRecord.getGiftAmount();
                }
            }
        } else {// 非收单退款
            BigDecimal refundRechargeAmount = BigDecimal.ZERO;
            BigDecimal refundGiftAmount = BigDecimal.ZERO;
            for (RefundOrder refundOrder : refundOrderList) {
                UserShoppingCreditsRecord refundRecord = userShoppingCreditsRecordService.getByLinkNoAndType(refundOrder.getRefundOrderNo(), "refund");
                refundRechargeAmount = refundRechargeAmount.add(refundRecord.getRechargeAmount());
                refundGiftAmount = refundRechargeAmount.add(refundRecord.getGiftAmount());
            }
            BigDecimal remainingRechargeAmount = orderRecord.getRechargeAmount().subtract(refundRechargeAmount);
            BigDecimal remainingGiftAmount = orderRecord.getGiftAmount().subtract(refundGiftAmount);
            if (remainingRechargeAmount.compareTo(BigDecimal.ZERO) == 0) {
                // 仅使用赠送购物金
                rechargeRefundAmount = new BigDecimal("0.00");
                giftRefundAmount = refundPrice;
            } else if (remainingGiftAmount.compareTo(BigDecimal.ZERO) == 0) {
                // 仅使用充值购物金
                rechargeRefundAmount = refundPrice;
                giftRefundAmount = new BigDecimal("0.00");
            } else {
                // 同时使用充值 + 赠送，先退赠送部分
                if (remainingGiftAmount.compareTo(refundPrice) >= 0) {
                    rechargeRefundAmount = new BigDecimal("0.00");
                    giftRefundAmount = refundPrice;
                } else {
                    rechargeRefundAmount = refundPrice.subtract(remainingGiftAmount);
                    giftRefundAmount = remainingGiftAmount;
                }
            }
        }

        UserShoppingCreditsRecord creditsRecord = new UserShoppingCreditsRecord();
        creditsRecord.setUid(userId);
        creditsRecord.setMerId(merId);
        creditsRecord.setLinkNo(refundOrderNo);
        creditsRecord.setLinkType("refund");
        creditsRecord.setType(1);
        creditsRecord.setTitle("订单退款");
        creditsRecord.setRechargeAmount(rechargeRefundAmount);
        creditsRecord.setGiftAmount(giftRefundAmount);
     ...
     * @param orderNo 订单单号
     */
    private List<RefundOrder> findByOrderNo(String orderNo) {
        LambdaQueryWrapper<RefundOrder> lqw = Wrappers.lambdaQuery();
        lqw.eq(RefundOrder::getOrderNo, orderNo);
        lqw.in(RefundOrder::getRefundStatus, 2, 3);
        return dao.selectList(lqw);
    }
     ...
     *
     * @param payNum         订单购买数量
     * @param refundNum      订单已退款数量
     * @param applyRefundNum 订单申请退款数量
     */
    private List<ReservationWorkOrder> findAwaitRefundWorkOrderList(Integer payNum, Integer refundNum, Integer applyRefundNum, String orderNo) {
        ...
        if (payNum.equals(applyRefundNum)) {// 整单退款
            workOrderList = reservationWorkOrderService.findByOrderNo(orderNo);
        } else if (payNum == (refundNum + applyRefundNum)) {// 最后一部分退款
            workOrderList = reservationWorkOrderService.findNoRefundByOrderNo(orderNo);
        } else { // 分单退款中间部分
            List<ReservationWorkOrder> noRefundList = reservationWorkOrderService.findNoRefundByOrderNo(orderNo);
            // 未开始服务工单
            List<ReservationWorkOrder> noStartList = noRefundList.stream().filter(e -> e.getServiceStatus() < 3).collect(Collectors.toList());
            ...
            // 已开始服务工单
            List<ReservationWorkOrder> startList = noRefundList.stream().filter(e -> e.getServiceStatus() >= 3).collect(Collectors.toList());
    ...
    private Integer getCircleRefundOrderCount(RefundOrderSearchRequest request, Integer status, List<Integer> merIds) {
        ...
        return dao.getCircleRefundOrderCount(map);
    }
}
