...
public class OrderController {
    ...
    @ApiOperation(value = "取消预约服务订单")
    @RequestMapping(value = "/cancel/reservation/{orderNo}", method = RequestMethod.POST)
    public CommonResult<Boolean> cancelReservation(@PathVariable(value = "orderNo") String orderNo) {
        if (orderService.cancelReservation(orderNo)) {
            return CommonResult.success();
        }
        return CommonResult.failed();
    }
