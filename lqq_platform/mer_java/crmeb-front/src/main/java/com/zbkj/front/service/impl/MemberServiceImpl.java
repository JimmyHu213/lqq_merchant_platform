...
public class MemberServiceImpl implements MemberService {
    ...
    private List<SvipCardResponse> getSvipCardList(Integer uid) {
        ...
        // 用户是否初次购买
        Boolean isFirstCharge = paidMemberOrderService.userIsFirstCharge(uid);
        List<SvipCardResponse> cardList = paidMemberCardService.findFrontList(isPayTrial);
        cardList.forEach(card -> {
            card.setIsFirstChargeGive(card.getIsFirstChargeGive() && isFirstCharge);
        });
        return cardList;
    }
