...
import com.zbkj.front.request.IndexMerchantRequest;
import com.zbkj.front.request.IndexProductComponentRequest;
...
public class IndexServiceImpl implements IndexService {
     ...
     */
    @Override
    public List<ProductCommonResponse> findIndexProductListComponent(IndexProductComponentRequest request) {
        String cids = "";
        String brandIds = "";
        String merIds = "";
        String proIds = "";
        if (StrUtil.isNotBlank(request.getCids()) && !request.getCids().equals("0")) {
            try {
                CrmebUtil.stringToArray(request.getCids());
            } catch (Exception e) {
                throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "商品分类格式错误");
            }
            cids = request.getCids();
        }
        if (StrUtil.isNotBlank(request.getBrandIds()) && !request.getBrandIds().equals("0")) {
            try {
                CrmebUtil.stringToArray(request.getBrandIds());
            } catch (Exception e) {
                throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "商品品牌格式错误");
            }
            brandIds = request.getBrandIds();
        }
        if (StrUtil.isNotBlank(request.getCircleIds()) && !request.getCircleIds().equals("0")) {
            List<Integer> merIdList;
            try {
                List<Integer> circleIdList = CrmebUtil.stringToArray(request.getCircleIds());
                merIdList = circleMerchantService.getMerIdListByCirCleIds(circleIdList);
            } catch (Exception e) {
                throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "圈层参数格式错误");
            }
            if (CollUtil.isNotEmpty(merIdList)) {
                merIds = StrUtil.join(",", merIdList);
            }
        } else if (StrUtil.isNotBlank(request.getProIds()) || StrUtil.isNotBlank(request.getMerIds())) {
            if (StrUtil.isNotBlank(request.getProIds())) {
                try {
                    CrmebUtil.stringToArray(request.getProIds());

                } catch (Exception e) {
                    throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "商品格式错误");
                }
                proIds = request.getProIds();
            }
            if (StrUtil.isNotBlank(request.getMerIds())) {
                try {
                    CrmebUtil.stringToArray(request.getMerIds());

                } catch (Exception e) {
                    throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "商户格式错误");
                }
                merIds = request.getMerIds();
            }
        }
        // 排序部分
        String lastStr;
        if (StrUtil.isNotBlank(request.getSalesOrder())) {
            if (request.getSalesOrder().equals(Constants.SORT_DESC)) {
                lastStr = " order by (p.sales + p.ficti) desc, p.rank desc, p.sort desc, p.id desc";
            } else {
                lastStr = " order by (p.sales + p.ficti) asc, p.rank desc, p.sort desc, p.id desc";
            }
        } else if (StrUtil.isNotBlank(request.getPriceOrder())) {
            if (request.getPriceOrder().equals(Constants.SORT_DESC)) {
                lastStr = " order by p.price desc, p.rank desc, p.sort desc, p.id desc";
            } else {
                lastStr = " order by p.price asc, p.rank desc, p.sort desc, p.id desc";
            }
        } else {
            lastStr = " order by p.rank desc, p.sort desc, p.id desc";
        }
        if (ObjectUtil.isNotNull(request.getNum()) && request.getNum() > 0) {
            lastStr = lastStr + " limit " + request.getNum();
        } else if (StrUtil.isBlank(proIds)) {
            lastStr = lastStr + " limit 10";
        }
        List<Product> productList = productService.findIndexProductListComponent(cids, brandIds, merIds, proIds, lastStr);
        if (CollUtil.isEmpty(productList)) {
     ...
     */
    @Override
    public List<IndexMerchantResponse> findIndexMerchantList(IndexMerchantRequest request) {
        ...
        } else if (StrUtil.isNotBlank(request.getMerIds()) && !request.getMerIds().equals("0")) {
            try {
                merIdList = CrmebUtil.stringToArray(request.getMerIds());
                num = merIdList.size();
            } catch (Exception e) {
                throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "商户参数格式错误");
            }
        }
        if (ObjectUtil.isNotNull(request.getNum())) {
            num = request.getNum();
        } else if (CollUtil.isNotEmpty(merIdList)) {
            num = merIdList.size();
        }
        List<Merchant> merchantList = merchantService.findIndexMerchantList(merIdList, num);
