...
public class UserCenterServiceImpl extends ServiceImpl<UserDao, User> implements UserCenterService {
    ...
    public UserCenterMenuResponse getUserCenterInfoMenu() {
        ...
        MerchantServiceStaff staff = merchantServiceStaffService.getByUserId(uid);
        if (ObjectUtil.isNotNull(staff) && staff.getStatus().equals(1)) {
            response.setIsServiceStaff(true);
        }
        return response;
    }
