...
public class ResponseRouter {
    ...
    public String filter(String data, String path, CrmebConfig crmebConfig) {
        ...
        if (!path.contains("api/admin/") && !path.contains("api/front/") && !path.contains("api/publicly/upload/")) {
            return data;
        }
        ...
        if ((data.contains(UploadConstants.UPLOAD_FILE_KEYWORD + "/") && !data.contains("data:image/png;base64"))
            ...
            if (data.contains(UploadConstants.UPLOAD_FILE_KEYWORD + "/" + UploadConstants.UPLOAD_FILE_TYPE_FILE) ||
                    data.contains(UploadConstants.UPLOAD_FILE_KEYWORD + "/" + UploadConstants.UPLOAD_FILE_TYPE_IMAGE)) {
                if (data.contains(UploadConstants.UPLOAD_FILE_KEYWORD + "/" + UploadConstants.UPLOAD_FILE_TYPE_FILE)) {
                    data = SpringUtil.getBean(SystemAttachmentService.class).prefixFile(data);
                }
                if (data.contains(UploadConstants.UPLOAD_FILE_KEYWORD + "/" + UploadConstants.UPLOAD_FILE_TYPE_IMAGE)) {
                    data = SpringUtil.getBean(SystemAttachmentService.class).prefixImage(data);
                }
            } else if (data.contains(UploadConstants.DOWNLOAD_FILE_KEYWORD + "/" + UploadConstants.UPLOAD_MODEL_PATH_EXCEL)) {
                data = SpringUtil.getBean(SystemAttachmentService.class).prefixFile(data);
...
//            if (data.contains(UploadConstants.UPLOAD_FILE_KEYWORD + "/" + UploadConstants.UPLOAD_FILE_TYPE_FILE)) {
//                data = SpringUtil.getBean(SystemAttachmentService.class).prefixFile(data);
//            } else if (data.contains(UploadConstants.UPLOAD_FILE_KEYWORD + "/" + UploadConstants.UPLOAD_FILE_TYPE_IMAGE)) {
//                data = SpringUtil.getBean(SystemAttachmentService.class).prefixImage(data);
//            } else if (data.contains(UploadConstants.DOWNLOAD_FILE_KEYWORD + "/" + UploadConstants.UPLOAD_MODEL_PATH_EXCEL)) {
//                data = SpringUtil.getBean(SystemAttachmentService.class).prefixFile(data);
//            } else if (data.contains(UploadConstants.UPLOAD_AFTER_FILE_KEYWORD + "/")) {
