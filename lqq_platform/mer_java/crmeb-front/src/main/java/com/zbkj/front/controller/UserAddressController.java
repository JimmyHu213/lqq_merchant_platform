...
public class UserAddressController {
    ...
    @ApiOperation(value = "获取用户地址经纬度")
    @RequestMapping(value = "/get/lon/lat/{id}", method = RequestMethod.GET)
    public CommonResult<Location> getLonLat(@PathVariable(value = "id") Integer id) {
        return CommonResult.success(userAddressService.getLonLatById(id));
    }

    @ApiOperation(value = "获取地图地址信息")
    @RequestMapping(value = "/map/info", method = RequestMethod.GET)
    public CommonResult<UserAddress> mapInfo(@ModelAttribute @Validated Location location) {
        return CommonResult.success(userAddressService.mapInfo(location));
    }
}
