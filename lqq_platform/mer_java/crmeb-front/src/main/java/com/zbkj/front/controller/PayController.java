...
public class PayController {
    ...
    @RequestMapping(value = "/get/config/{orderNo}", method = RequestMethod.GET)
    public CommonResult<PayConfigResponse> getPayConfig(@PathVariable("orderNo") String orderNo) {
        return CommonResult.success(payService.getPayConfig(orderNo));
    }
    ...
    @ApiOperation(value = "查询微信商家转账结果")
    @RequestMapping(value = "/query/wechat/transfer/result/{orderNo}", method = RequestMethod.GET)
    public CommonResult<String> queryWechatTransferResult(@PathVariable("orderNo") String orderNo) {
        if (payService.queryWechatTransferResult(orderNo)) {
            return CommonResult.success("查询成功");
        }
        return CommonResult.success("请稍后重新查询");
    }
}
