...
public class FrontOrderServiceImpl implements FrontOrderService {
    ...
    public OrderNoResponse preOrder_V1_7(PreOrderRequest request) {
        ...
        BigDecimal svipDiscountTotalPrice = BigDecimal.ZERO;
        BigDecimal merchantMemberDiscountTotalPrice = BigDecimal.ZERO;
        ...
        for (PreMerchantOrderVo merchantOrderVo : preOrderInfoVo.getMerchantOrderVoList()) {
            ...
            merchantOrderVo.setMerchantMemberDiscountPrice(BigDecimal.ZERO);
            boolean isMerchantMember = false;
            MerchantMember merchantMember = merchantMemberService.getByUidAndMerId(user.getId(), merchantOrderVo.getMerId());
            ...
            if (user.getIsPaidMember() && preOrderInfoVo.getType().equals(OrderConstants.ORDER_TYPE_BASE)) {
                BigDecimal merSvipTotalPrice = merchantOrderVo.getOrderInfoList().stream()
                        ...
                        .reduce(BigDecimal.ZERO, BigDecimal::add);
                BigDecimal svipDiscountPrice = merTotalPrice.subtract(merSvipTotalPrice);
                if (svipDiscountPrice.compareTo(BigDecimal.ZERO) > 0) {
                    merchantOrderVo.setSvipDiscountPrice(svipDiscountPrice);
                    svipDiscountTotalPrice = svipDiscountTotalPrice.add(merchantOrderVo.getSvipDiscountPrice());
                }
            }
            if (isMerchantMember && preOrderInfoVo.getType().equals(OrderConstants.ORDER_TYPE_BASE)) {
                BigDecimal merMemberTotalPrice = merchantOrderVo.getOrderInfoList().stream()
                        .map(e -> {
                            BigDecimal price;
                            if (e.getVipPriceType().equals(2)) {
                                price = e.getVipPrice().multiply(new BigDecimal(e.getPayNum()));
                            } else {
                                price = e.getPrice().multiply(new BigDecimal(e.getPayNum()));
                            }
                        ...
                        .reduce(BigDecimal.ZERO, BigDecimal::add);
                BigDecimal merMemberDiscountPrice = merTotalPrice.subtract(merMemberTotalPrice);
                if (merMemberDiscountPrice.compareTo(BigDecimal.ZERO) > 0) {
                    merchantOrderVo.setMerchantMemberDiscountPrice(merMemberDiscountPrice);
                    merchantMemberDiscountTotalPrice = merchantMemberDiscountTotalPrice.add(merchantOrderVo.getMerchantMemberDiscountPrice());
                }
            }
        ...
        if (preOrderInfoVo.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CLOUD)
                ...
                || preOrderInfoVo.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_RESERVATION)
                || preOrderInfoVo.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CARDTIMES)) {
        ...
        // 实际支付金额
        preOrderInfoVo.setSvipDiscountPrice(svipDiscountTotalPrice);
        preOrderInfoVo.setMerchantMemberDiscountPrice(merchantMemberDiscountTotalPrice);
        preOrderInfoVo.setPayFee(preOrderInfoVo.getProTotalFee().add(preOrderInfoVo.getFreightFee())
                .subtract(preOrderInfoVo.getSvipDiscountPrice()).subtract(preOrderInfoVo.getMerchantMemberDiscountPrice()));
        ...
        BigDecimal proPayFee = preOrderInfoVo.getProTotalFee().subtract(preOrderInfoVo.getCouponFee())
                .subtract(preOrderInfoVo.getSvipDiscountPrice()).subtract(preOrderInfoVo.getMerchantMemberDiscountPrice());
        ...
        OrderNoResponse preOrderResponse = createPreOrderResponse(user.getId(), preOrderInfoVo);
        merchantUserService.addManyMerchantUser(user.getId(), merIdList);
        return preOrderResponse;
    }
    ...
    private void preOrderSetCouponPrice(PreOrderInfoVo preOrderInfoVo, List<PreOrderInfoDetailVo> orderInfoList, User user) {
        ...
        for (PreMerchantOrderVo merchantOrderVo : preOrderInfoVo.getMerchantOrderVoList()) {
            ...
            merIdList.add(merId);
            boolean isMerchantMember;
            MerchantMember merchantMember = merchantMemberService.getByUidAndMerId(user.getId(), merId);
            if (ObjectUtil.isNotNull(merchantMember) && !merchantMember.getIsLogoff()) {
                isMerchantMember = true;
            } else {
                isMerchantMember = false;
            }
            ...
            BigDecimal merPrice = merchantOrderVo.getProTotalFee().subtract(merchantOrderVo.getSvipDiscountPrice()).subtract(merchantOrderVo.getMerchantMemberDiscountPrice());
            ...
            for (int i = 0; i < merCouponList.size(); ) {
                ...
                BigDecimal proPrice = detailVoList.stream()
                        .map(e -> {
                            BigDecimal price;
                            if (e.getVipPriceType().equals(1) && user.getIsPaidMember()) {
                                price = e.getVipPrice().multiply(new BigDecimal(e.getPayNum()));
                            } else if (e.getVipPriceType().equals(2) && isMerchantMember) {
                                price = e.getVipPrice().multiply(new BigDecimal(e.getPayNum()));
                            } else {
                                price = e.getPrice().multiply(new BigDecimal(e.getPayNum()));
                            }
            ...
            for (int i = 0; i < merCouponList.size(); ) {
                ...
                if (!couponUserService.userIsCanReceiveCoupon(merCouponList.get(i), user.getId(), isMerchantMember)) {
                    merCouponList.remove(i);
                    continue;
                }
                i++;
        ...
        // 自动领平台券计算
        // 查所有适用的平台优惠券
        BigDecimal remainPrice = preOrderInfoVo.getProTotalFee().subtract(preOrderInfoVo.getSvipDiscountPrice())
                .subtract(preOrderInfoVo.getMerchantMemberDiscountPrice()).subtract(preOrderInfoVo.getMerCouponFee());
    ...
    private void setMerProCouponPrice(PreMerchantOrderVo merchantOrderVo, Coupon coupon, CouponUser couponUser, Boolean userIsPaidMember) {
        ...
        BigDecimal proTotalPrice = proDtoList.stream()
                .map(e -> {
                    BigDecimal price;
                    if (e.getVipPriceType() > 0 && userIsPaidMember) {
                        price = e.getVipPrice().multiply(new BigDecimal(e.getPayNum()));
                    } else {
                        price = e.getPrice().multiply(new BigDecimal(e.getPayNum()));
                    }
        ...
        for (int i = 0; i < proDtoList.size(); i++) {
            ...
            BigDecimal proPrice;
            if (d.getVipPriceType() > 0 && userIsPaidMember) {
    ...
    public PreOrderResponse loadPreOrder(String preOrderNo) {
        ...
        for (PreMerchantOrderVo merchantOrderVo : merchantOrderVoList) {
            ...
            boolean isMerchantMember;
            MerchantMember merchantMember = merchantMemberService.getByUidAndMerId(userId, merchantOrderVo.getMerId());
            if (ObjectUtil.isNotNull(merchantMember) && !merchantMember.getIsLogoff()) {
                isMerchantMember = true;
            } else {
                isMerchantMember = false;
        ...
        if (orderInfoVo.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_RESERVATION)) {
            PreMerchantOrderVo merchantOrderVo = merchantOrderVoList.get(0);
            preOrderResponse.setReservationDate(merchantOrderVo.getReservationDate());
            preOrderResponse.setReservationTimeSlot(merchantOrderVo.getReservationTimeSlot());
            preOrderResponse.setFormDataSubmitType(merchantOrderVo.getFormDataSubmitType());
        }
        preOrderResponse.setMerchantInfoList(infoResponseList);
        ...
        for (CouponUser couponUser : platCouponUserList) {
            ...
            if (couponUser.getCategory().equals(CouponConstants.COUPON_CATEGORY_PRODUCT)) {
                ...
                BigDecimal proPrice = orderDetailInfoList.stream().filter(f -> cpIdList.contains(f.getProductId()))
                        .map(e -> {
                            BigDecimal price;
                            if (e.getVipPriceType() > 0 && user.getIsPaidMember()) {
                                price = e.getVipPrice().multiply(new BigDecimal(e.getPayNum()));
                            } else {
                                price = e.getPrice().multiply(new BigDecimal(e.getPayNum()));
                            }
    ...
    public OrderNoResponse createOrder(CreateOrderRequest orderRequest) {
        ...
        if (orderInfoVo.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CLOUD)
                ...
                || orderInfoVo.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CARDTIMES)) {
            orderRequest.setAddressId(0);
        } else if (orderInfoVo.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_RESERVATION)) {
            OrderMerchantRequest orderMerchantRequest = orderMerchantRequestList.get(0);
            if (orderMerchantRequest.getShippingType().equals(OrderConstants.ORDER_SHIPPING_TYPE_HOME_SERVICE)) {
                if (ObjectUtil.isNull(orderRequest.getAddressId())) {
                    throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "请选择预约地址");
                }
                userAddress = userAddressService.getById(orderRequest.getAddressId());
                if (ObjectUtil.isNull(userAddress) || userAddress.getIsDel()) {
                    throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "预约地址有误");
                }
            }
        } else {
        ...
        if (orderRequest.getIsUseIntegral() && user.getIntegral() > 0) {// 使用积分
            integralDeductionComputed(orderInfoVo, user.getIntegral(), user.getIsPaidMember(), user.getId());
        }
        List<PreMerchantOrderVo> merchantOrderVoList = orderInfoVo.getMerchantOrderVoList();
        ...
        order.setMerchantMemberDiscountPrice(orderInfoVo.getMerchantMemberDiscountPrice());
        ...
        order.setPayPrice(order.getTotalPrice().subtract(order.getSvipDiscountPrice()).subtract(order.getMerchantMemberDiscountPrice())
                .subtract(order.getCouponPrice()).subtract(order.getIntegralPrice()));
        ...
        if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_RESERVATION)) {
            order.setCancelServiceOrderSwitch(orderInfoVo.getCancelServiceOrderSwitch());
            order.setCancelServiceOrderHour(orderInfoVo.getCancelServiceOrderHour());
            ...
            if (StrUtil.isNotBlank(orderRequest.getOrderExtend())) {
                String orderExtend = orderRequest.getOrderExtend();
                JSONArray extendJsonArray = JSONArray.parseArray(orderExtend);
                JSONArray extendJsonArray0 = extendJsonArray.getJSONArray(0);
                order.setOrderExtend(systemAttachmentService.clearPrefix(extendJsonArray0.toJSONString(), systemAttachmentService.getCdnUrl()));
            } else {
                order.setOrderExtend("");
            }
        }
        ...
        for (PreMerchantOrderVo merchantOrderVo : merchantOrderVoList) {
            ...
            } else if (merchantOrder.getShippingType().equals(OrderConstants.ORDER_SHIPPING_TYPE_TO_STORE)) {
                merchantOrder.setRealName(orderRequest.getReservationName());
                merchantOrder.setUserPhone(orderRequest.getReservationPhone());
                merchantOrder.setVerifyCode(String.valueOf(CrmebUtil.randomCount(1111111111, 999999999)));
            } else {
            ...
            merchantOrder.setMerchantMemberDiscountPrice(merchantOrderVo.getMerchantMemberDiscountPrice());
            ...
            merchantOrder.setPayPrice(merchantOrder.getTotalPrice().subtract(merchantOrder.getSvipDiscountPrice())
                    .subtract(merchantOrder.getMerchantMemberDiscountPrice()).subtract(merchantOrder.getCouponPrice())
                    .subtract(merchantOrder.getIntegralPrice()));
            ...
            if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_RESERVATION)) {
                merchantOrder.setReservationDate(merchantOrderVo.getReservationDate());
                ProductAttrValueSupplementReservation attrValueReservation = productAttrValueSupplementReservationService.getById(merchantOrderVo.getAttrValueReservationId());
                merchantOrder.setReservationTimeSlot(attrValueReservation.getTimeSlot());
            }

            merchantOrderList.add(merchantOrder);

            boolean isMerchantMember;
            MerchantMember merchantMember = merchantMemberService.getByUidAndMerId(user.getId(), merchantOrder.getMerId());
            if (ObjectUtil.isNotNull(merchantMember) && !merchantMember.getIsLogoff()) {
                isMerchantMember = true;
            } else {
                isMerchantMember = false;
            ...
            for (PreOrderInfoDetailVo detailVo : detailVoList) {
                ...
                orderDetail.setVipPriceType(detailVo.getVipPriceType());
                ...
                if (orderDetail.getIsSvip() && orderDetail.getVipPriceType().equals(1)) {
                ...
                } else if (orderDetail.getVipPriceType().equals(2) && isMerchantMember)  {
                    detailPayPrice = orderDetail.getVipPrice().multiply(new BigDecimal(orderDetail.getPayNum().toString())).add(orderDetail.getFreightFee()).subtract(orderDetail.getCouponPrice()).subtract(orderDetail.getIntegralPrice());
                } else {
                ...
                if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CARDTIMES)) {
                    orderDetail.setVerifyTotalTimes(detailVo.getVerifyTotalTimes());
                    orderDetail.setVerifyRemainingTimes(detailVo.getVerifyRemainingTimes());
                    orderDetail.setVerifyStartDate(detailVo.getVerifyStartDate());
                    orderDetail.setVerifyEndDate(detailVo.getVerifyEndDate());
                }
            ...
            if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_RESERVATION)) {
                ...
                for (OrderDetail orderDetail : orderDetailList) {
                    orderDetail.setReservationDate(merchantOrderVo.getReservationDate());
                    orderDetail.setReservationTimeSlot(attrValueReservation.getTimeSlot());
                    orderDetail.setReservationFormData(StrUtil.isNotBlank(orderRequest.getOrderExtend()) ? systemAttachmentService.clearPrefix(orderRequest.getOrderExtend(), systemAttachmentService.getCdnUrl()) : "");
                }
            }
        }
...
//                payComponentProductService.operationStock(skuRecord.getInt("productId"), skuRecord.getInt("num"), Constants.OPERATION_TYPE_SUBTRACT);
            ...
            if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_NORMAL)
                    ...
                    || order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_VIRTUALLY)
                    || order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CARDTIMES)) { // 普通商品
                // 扣减库存
            ...
            if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_RESERVATION)) {
                // 预约商品只增加销量
                MyRecord skuRecord = skuRecordList.get(0);
                productService.operationSales(skuRecord.getInt("productId"), skuRecord.getInt("num"), Constants.OPERATION_TYPE_ADD);
                productAttrValueService.operationSales(skuRecord.getInt("attrValueId"), skuRecord.getInt("num"), Constants.OPERATION_TYPE_ADD);
            }
            ...
            // 生成订单日志
            orderFlowRecordService.orderCreate(order.getOrderNo(), order.getUid());
            // 清除购物车数据
     ...
     */
    private void integralDeductionComputed(PreOrderInfoVo orderInfoVo, Integer userIntegral, Boolean userIsPaidMember, Integer userId) {
        BigDecimal payPrice = orderInfoVo.getProTotalFee().subtract(orderInfoVo.getSvipDiscountPrice())
                .subtract(orderInfoVo.getMerchantMemberDiscountPrice()).subtract(orderInfoVo.getCouponFee());
        ...
        List<PreMerchantOrderVo> voList = merchantOrderVoList.stream()
                .sorted(Comparator.comparing(o -> (o.getProTotalFee().subtract(o.getSvipDiscountPrice())
                        .subtract(o.getMerchantMemberDiscountPrice()).subtract(o.getCouponFee()))))
        ...
        for (int i = 0; i < voList.size(); i++) {
            ...
            BigDecimal merPayPrice = merchantOrderVo.getProTotalFee().subtract(merchantOrderVo.getSvipDiscountPrice())
                    .subtract(merchantOrderVo.getMerchantMemberDiscountPrice()).subtract(merchantOrderVo.getMerCouponFee());
            if (merchantOrderVoList.size() == (i + 1)) {
                ...
                orderInfoList = orderInfoList.stream()
                        .sorted(Comparator.comparing(o -> {
                            BigDecimal oPrice;
                            if (o.getVipPriceType().equals(1) && userIsPaidMember) {
                                oPrice = o.getVipPrice().multiply(new BigDecimal(o.getPayNum())).subtract(o.getMerCouponPrice());
                            } else if (o.getVipPriceType().equals(2) && isMerchantMember) {
                                oPrice = o.getVipPrice().multiply(new BigDecimal(o.getPayNum())).subtract(o.getMerCouponPrice());
                            } else {
     ...
     */
    @Override
    public Boolean cancelReservation(String orderNo) {
        Integer userId = userService.getUserIdException();
        ...
        if (order.getRefundStatus() > 0) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "已申请退款订单不可取消");
        }
        // 判断订单是否支持取消
        if (!order.getCancelServiceOrderSwitch()) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "此订单不支持取消预约");
        }
        List<OrderDetail> orderDetailList = orderDetailService.getByOrderNo(orderNo);
        OrderDetail orderDetail = orderDetailList.get(0);
        // 判断日期是否过期
        DateTime date = DateUtil.date();
        DateTime reservationDate = DateUtil.parse(orderDetail.getReservationDate());
        long betweenDay = DateUtil.between(DateUtil.beginOfDay(date), reservationDate, DateUnit.DAY, false);
        if (betweenDay < 0) {
            // 已经超过可预约日期
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "此订单已超过取消预约日期");
        }
        // 判断时间是否过期
        Integer hour = order.getCancelServiceOrderHour();
        String timeSlot = orderDetail.getReservationTimeSlot();
        String[] split = timeSlot.split("-");
        String startTime = split[0];
        DateTime reservationStartDate = DateUtil.parse(orderDetail.getReservationDate() + " " + startTime);
        double exactHours = DateUtil.between(date, reservationStartDate, DateUnit.HOUR, false);
        if (exactHours < (double) hour) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "此订单已超过取消预约时间");
        }
        orderDetailList.forEach(od -> od.setApplyRefundNum(od.getPayNum()));
        // 取消预约服务订单 = 自动申请退款，自动通过审核，自动退款
        Boolean isExist = reservationWorkOrderService.isExistWorkByOrderNo(orderNo);
        if (isExist) {
            List<Integer> workOrderIdList = reservationWorkOrderService.findIdByOrderNo(orderNo);
            return refundOrderService.merchantDirectRefund(order, orderDetailList, workOrderIdList);
        }
        return refundOrderService.merchantDirectRefund(order, orderDetailList, new ArrayList<>());
    }
    ...
    public OrderFrontDetailResponse frontDetail(String orderNo) {
        ...
        for (MerchantOrder merchantOrder : merchantOrderList) {
            ...
            if (merchantOrder.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_RESERVATION)) {
                ...
                merDetailResponse.setReservationDate(orderDetail.getReservationDate());
                merDetailResponse.setReservationTimeSlot(orderDetail.getReservationTimeSlot());
                merDetailResponse.setReservationFormData(orderDetail.getReservationFormData());
                ...
                if (order.getCancelServiceOrderSwitch()) {
                    ...
                    if ((betweenDay >= 0) && (exactHours >= (double) hour)) {
                        response.setIsCancelServiceOrder(true);
                    }
                }
                // 工单服务人员信息
                List<ReservationWorkOrder> reservationWorkOrders = reservationWorkOrderService.findByOrderNo(orderNo);
                if (CollUtil.isNotEmpty(reservationWorkOrders)) {
                    List<Integer> staffIds = reservationWorkOrders.stream()
                            .filter(wo -> ObjectUtil.isNotNull(wo.getServiceStaffId()) && wo.getServiceStaffId() > 0)
                            .map(ReservationWorkOrder::getServiceStaffId)
                            .collect(Collectors.toList());
                    if (CollUtil.isNotEmpty(staffIds)) {
                        List<MerchantServiceStaff> StaffList = merchantServiceStaffService.findByStaffIdList(staffIds);
                        dataResponseList.get(0).setStaffList(StaffList);
                    }
                }
            }
            if (merchantOrder.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CARDTIMES)) {
                OrderDetail orderDetail = detailList.get(0);
                // 次卡商品是否可以申请退款
                boolean cardIfRefund = Boolean.FALSE;
                if (Objects.equals(orderDetail.getVerifyRemainingTimes(), orderDetail.getVerifyTotalTimes())
                        && (StrUtil.isBlank(orderDetail.getVerifyEndDate())
                        || DateUtil.parseDate(orderDetail.getVerifyEndDate()).after(DateUtil.date()))) {
                    cardIfRefund = Boolean.TRUE;
                }
                response.setCardIfRefund(cardIfRefund);
            }
            // 商户门店图片
            if (merchantOrder.getMerId() > 0) {
                merDetailResponse.setCoverImage(merchantService.getByIdException(merchantOrder.getMerId()).getCoverImage());
            }
    ...
    public Boolean cancel(String orderNo) {
        ...
        if (cancel) {
            orderFlowRecordService.orderCancel(order.getOrderNo(), order.getUid(), "user");
    ...
    public Boolean takeDelivery(String orderNo) {
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            orderFlowRecordService.orderReceipt(order.getOrderNo(), order.getUid(), "user", null);
            return Boolean.TRUE;
        });
        if (execute) {
            //后续操作放入redis
    ...
    public Boolean refundApply(OrderRefundApplyRequest request) {
        ...
        if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_RESERVATION)
                && !request.getAfterSalesType().equals(1)) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "预约订单只能申请仅退款");
        }
        ...
        if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CARDTIMES)) {
            if (!orderDetail.getPayNum().equals(request.getNum())) {
                throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "次卡订单退款数量必须是全数退款");
            }
            if (orderDetail.getVerifyRemainingTimes() < orderDetail.getVerifyTotalTimes()) {
                throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "次卡订单已经核销过，无法申请退款");
            }
            ...
            if (StrUtil.isNotBlank(verifyEndDate) && today.compareTo(verifyEndDate) > 0) {
                throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "次卡订单已过期，无法申请退款,订单号=" + order.getOrderNo());
            }
            if (orderDetail.getVerifyRemainingTimes() <= 0) {
                throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "次卡订单已核销完毕，无法申请退款,订单号=" + order.getOrderNo());
            }
        }
        ...
        refundOrderInfo.setRefundVerifyTimes(orderDetail.getVerifyRemainingTimes());
        // 临时性计算退款金额、积分
...
//            return recordList;
        ...
        if (orderInfoVo.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_INTEGRAL)) {
            ...
            orderInfoList.forEach(info -> {
                ...
                record.set("vipPriceType", product.getVipPriceType());
        ...
        merchantOrderVoList.forEach(merchantOrderVo -> {
            ...
            merchantOrderVo.getOrderInfoList().forEach(info -> {
                ...
                // TODO 预约商品库存校验，通过预约日期+预约时段，查询对应的订单销量，进行库存计算
    ...
    public void getFreightFee_V_1_8(PreOrderInfoVo orderInfoVo, UserAddress userAddress, Boolean userIsPaidMember) {
        ...
        for (PreMerchantOrderVo merchantOrderVo : merchantOrderVoList) {
            ...
            boolean isMerchantMember;
            MerchantMember merchantMember = merchantMemberService.getByUidAndMerId(userAddress.getUid(), merchantOrderVo.getMerId());
            if (ObjectUtil.isNotNull(merchantMember) && !merchantMember.getIsLogoff()) {
                isMerchantMember = true;
            } else {
                isMerchantMember = false;
            ...
            merchantOrderVo.getOrderInfoList().forEach(e -> {
                ...
                BigDecimal proPrice;
                if (e.getVipPriceType().equals(1) && userIsPaidMember) {
    ...
    private PreOrderInfoVo validatePreOrderRequest_V1_7(PreOrderRequest request, User user) {
        ...
        switch (request.getPreOrderType()) {
            ...
            case OrderConstants.PLACE_ORDER_TYPE_BUY_NOW:
                ...
                if (preMerchantOrderVo.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_RESERVATION)) {
                    ProductAttrValueSupplementReservation attrValueReservation = productAttrValueSupplementReservationService.getById(preOrderDetailRequest.getAttrValueReservationId());
                    preMerchantOrderVo.setReservationDate(preOrderDetailRequest.getReservationDate());
                    preMerchantOrderVo.setAttrValueReservationId(preOrderDetailRequest.getAttrValueReservationId());
                    preMerchantOrderVo.setReservationTimeSlot(attrValueReservation.getTimeSlot());
                    preOrderInfoVo.setCancelServiceOrderSwitch(preMerchantOrderVo.getCancelServiceOrderSwitch());
                    preOrderInfoVo.setCancelServiceOrderHour(preMerchantOrderVo.getCancelServiceOrderHour());
                }
...
//                break;
    ...
    private PreMerchantOrderVo validatePreOrderBase(PreOrderDetailRequest detailRequest) {
        ...
        if (product.getType().equals(ProductConstants.PRODUCT_TYPE_RESERVATION)) {
            validateReservationProduct(detailRequest);
        }
        // 次卡商品校验
        if (product.getType().equals(ProductConstants.PRODUCT_TYPE_CARDTIMES)) {
            String today = DateUtil.format(new Date(), DateConstants.DATE_FORMAT_DATE);
            if (product.getVerifyTimeLimitType().equals(ProductConstants.CARD_TIMES_VALIDITY_LIMIT_DATE)
            && today.compareTo(product.getVerifyTimeLimitEndDate()) > 0) {
                throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "该次卡商品的核销期限小于今天");
            }
        }
        ...
        if (product.getType().equals(ProductConstants.PRODUCT_TYPE_RESERVATION)) {
            ...
            merchantOrderVo.setCancelServiceOrderSwitch(supplementReservation.getCancelOrderSwitch());
            merchantOrderVo.setCancelServiceOrderHour(supplementReservation.getCancelOrderHour());
            merchantOrderVo.setFormDataSubmitType(supplementReservation.getFormDataSubmitType());
        }
        ...
        detailVo.setVipPriceType(product.getVipPriceType());
        // 次卡商品数据填充
        if (product.getType().equals(ProductConstants.PRODUCT_TYPE_CARDTIMES)) {
            detailVo.setVerifyTotalTimes(product.getVerifyQuantity()*detailVo.getPayNum());
            detailVo.setVerifyRemainingTimes(detailVo.getVerifyTotalTimes());
            String startDate = "";
            String endDate = "";
            if (product.getVerifyTimeLimitType() == 2) {
                startDate = DateUtil.format(new Date(), "yyyy-MM-dd");
                endDate = DateUtil.format(DateUtil.offsetDay(new Date(), product.getVerifyTimeLimitDay()), "yyyy-MM-dd");
            }else if (product.getVerifyTimeLimitType() == 3) {
                startDate = product.getVerifyTimeLimitStartDate();
                endDate = product.getVerifyTimeLimitEndDate();
            }
            detailVo.setVerifyStartDate(startDate);
            detailVo.setVerifyEndDate(endDate);
        }
        if (product.getVipPriceType() > 0) {
        ...
        if (product.getType().equals(ProductConstants.PRODUCT_TYPE_RESERVATION)) {
            detailVo.setAttrValueReservationId(detailRequest.getAttrValueReservationId());
        }
        ...
        if (product.getType().equals(ProductConstants.PRODUCT_TYPE_CLOUD) || product.getType().equals(ProductConstants.PRODUCT_TYPE_CDKEY)) {
            merchantOrderVo.setShippingType(OrderConstants.ORDER_SHIPPING_TYPE_VIRTUAL);
        } else {
            if (product.getDeliveryMethod().contains("4")) {
                merchantOrderVo.setShippingType(OrderConstants.ORDER_SHIPPING_TYPE_HOME_SERVICE);
            } else if (product.getDeliveryMethod().contains("3")) {
                merchantOrderVo.setShippingType(OrderConstants.ORDER_SHIPPING_TYPE_TO_STORE);
            } else if (product.getDeliveryMethod().equals("2")) {
                merchantOrderVo.setShippingType(OrderConstants.ORDER_SHIPPING_TYPE_PICK_UP);
            } else {
                merchantOrderVo.setShippingType(OrderConstants.ORDER_SHIPPING_TYPE_EXPRESS);
            }
        }
     ...
     * 预约商品校验
     */
    private void validateReservationProduct(PreOrderDetailRequest detailRequest) {
        ProductAttrValueSupplementReservation attrValueReservation = productAttrValueSupplementReservationService.getById(detailRequest.getAttrValueReservationId());
        if (ObjectUtil.isNull(attrValueReservation)) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "商品规格时段信息不存在，请刷新后重新选择");
        }
        String reservationDate = detailRequest.getReservationDate();
        ProductSupplementReservation supplementReservation = productSupplementReservationService.getByProId(detailRequest.getProductId());
        ...
        if (reservationDateType == 2) {
            ...
            String reservationDateWeekRange = supplementReservation.getReservationDateWeekRange();
        ...
        // 根据日期 + 时段，从订单中获取对应销量，判断预约库存是否足够
        Integer salesNum = orderDetailService.getSalesNumCountByReservationIngo(attrValueReservation.getAttValueId(), reservationDate, attrValueReservation.getTimeSlot());
        if (attrValueReservation.getStock() <= salesNum) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "商品规格库存不足，请刷新后重新选择");
        }
        if (attrValueReservation.getStock() - salesNum < detailRequest.getProductNum()) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "商品规格库存不足，请刷新后重新选择");
        }
    }
    ...
    private ComputedOrderPriceResponse computedPrice(OrderComputedPriceRequest request, PreOrderInfoVo orderInfoVo, User user) {
        ...
        priceResponse.setMerchantMemberDiscountPrice(orderInfoVo.getMerchantMemberDiscountPrice());
        // 优惠券计算
        orderInfoVo.setCouponFee(BigDecimal.ZERO);
        ...
        // 积分部分
        BigDecimal payPrice = orderInfoVo.getProTotalFee().subtract(orderInfoVo.getSvipDiscountPrice())
                .subtract(orderInfoVo.getMerchantMemberDiscountPrice()).subtract(priceResponse.getCouponFee());
    ...
    private List<ComputedMerchantOrderResponse> computedPriceGetMerOrderList(PreOrderInfoVo orderInfoVo, Boolean userIsPaidMember) {
        ...
        List<ComputedMerchantOrderResponse> merOrderResponseList = orderInfoVo.getMerchantOrderVoList().stream().map(vo -> {
            ...
            boolean isMerchantMember;
            MerchantMember merchantMember = merchantMemberService.getByUidAndMerId(userId, vo.getMerId());
            if (ObjectUtil.isNotNull(merchantMember) && !merchantMember.getIsLogoff()) {
                isMerchantMember = true;
            } else {
                isMerchantMember = false;
            ...
            merOrderResponse.setMerchantMemberDiscountPrice(vo.getMerchantMemberDiscountPrice());
            // 查询适用的用户优惠券
            ...
            BigDecimal merPrice = vo.getProTotalFee().subtract(vo.getSvipDiscountPrice()).subtract(vo.getMerchantMemberDiscountPrice());
            BigDecimal merRemainingAmount = merPrice.subtract(orderInfoVo.getPlatCouponFee());
    ...
    private void getCouponFee_V1_3(PreOrderInfoVo orderInfoVo, Integer uid, Boolean userIsPaidMember) {
        ...
        for (PreMerchantOrderVo merchantOrderVo : merchantOrderVoList) {
            ...
            BigDecimal subProTotalFee = merchantOrderVo.getProTotalFee().subtract(merchantOrderVo.getSvipDiscountPrice())
                    .subtract(merchantOrderVo.getMerchantMemberDiscountPrice());
...
//                throw new CrmebException("总金额小于优惠券最小使用金额");
            ...
            boolean isMerchantMember;
            MerchantMember merchantMember = merchantMemberService.getByUidAndMerId(uid, merchantOrderVo.getMerId());
            if (ObjectUtil.isNotNull(merchantMember) && !merchantMember.getIsLogoff()) {
                isMerchantMember = true;
            } else {
                isMerchantMember = false;
