<template>
  ...
  <div v-if="configData" class="borderPadding">
    <div v-if="isShowTitle" class="acea-row">
    ...
    <draggable class="dragArea list-group" :list="configData.list" group="peoples" handle=".iconfont">
      <div class="box-item" v-for="(item, index) in configData.list" :key="index">
          ...
          <div class="c_row-item mb10">
            ...
            <el-col :span="19" class="slider-box">
              <el-input
                size="small"
                v-model="item.name"
                :maxlength="!isDiy ? 10 :4"
                :placeholder="`不超过${!isDiy ? 10 :4}个字`"
              />
            </el-col>
          </div>
          <div v-if="isDiy" class="c_row-item">
        ...
        <div v-if="isOperation(item)" class="del-box" @click="deleteMenu(index)">
    ...
    <el-button
      ...
      plain
      @click="addMenu"
      v-if="(configData.list && configData.list.length < 5) || (!isDiy && configData.list && configData.list.length < 20)"
      >添加</el-button
    >
...
import { merchantConfigInfoApi } from '@/api/merchant';
export default {
  name: 'c_foot',
  props: {
    // 是否是diy中使用
  ...
  methods: {
    getData() {
      this.$emit('getSaveData', this.configData);
    },
    // 是否能操作，
    isOperation(item) {
      return item.canDel || !item.type;
    },
    linkUrl(e) {
