...
import {couponUsableApi} from "@/api/member";
export default {
  name: 'CouponList',
  props: {
    handle: {
  ...
  methods: {
    ...
    async getList(num) {
      ...
      // this.tableFrom.type = this.userType === 'user' ? 1 : 3;
      try {
        const res = this.userType === 'member' ? await couponUsableApi() : await productCouponListApi();
