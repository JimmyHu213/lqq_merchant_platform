        ...
        <div class="form-data">
          <el-form :model="merInfoForm" :rules="rules" ref="merInfoForm" label-width="140px" class="demo-ruleForm">
            ...
            <el-form-item label="商户地址：" prop="addressDetail">
              ...
              <iframe id="mapPage" width="100%" height="500px" frameborder="0" :src="mapPickerUrl"  :key="mapPickerKey"/>
            </el-form-item>
...
export default {
  ...
  computed: {
    ...
    invalidCoord() {
      ...
      if (
          this.merInfoForm.latitude === '' ||
          this.merInfoForm.latitude === null ||
          this.merInfoForm.longitude === '' ||
          this.merInfoForm.longitude === null ||
          this.merInfoForm.latitude === undefined ||
          this.merInfoForm.longitude === undefined
      ) {
        return true;
      }
      const lat = parseFloat(this.merInfoForm.latitude);
      const lng = parseFloat(this.merInfoForm.longitude);
    ...
    mapPickerKey() {
      // 确保经纬度为字符串格式，避免精度问题
      const lat = this.merInfoForm.latitude ? String(this.merInfoForm.latitude) : '0';
      const lng = this.merInfoForm.longitude ? String(this.merInfoForm.longitude) : '0';
    ...
    mapPickerUrl() {
      ...
      if (
        ...
        const latStr = String(this.merInfoForm.latitude).trim();
        const lngStr = String(this.merInfoForm.longitude).trim();
