      ...
      <div class="acea-row row-baseline">
        ...
        <el-popover placement="bottom" width="450" trigger="click" v-model="visible">
          <div class="acea-row row-middle">
            ...
            >
            <el-button
              v-hasPermi="['merchant:page:diy:update', 'merchant:page:diy:save']"
              type="primary"
              size="small"
...
import { getBaseInfoApi } from '@/api/merchant';
...
export default {
  ...
  mounted() {
    ...
    this.getQRcode();
    if (!localStorage.getItem('JavaMerchantBaseInfo')) this.getBaseInfoApi();
  },
  beforeDestroy() {
    //销毁时移除监听
  ...
  methods: {
    // 获取商户配置信息
    async getBaseInfoApi() {
      let res = await getBaseInfoApi();
    ...
    getQRcode() {
      ...
      let frontDomains = `${this.$selfUtil.getFrontDomainUrl()}/pages/merchant/home/index?merId=${this.merchantId}`;
      new QRcode('diyQrcode', { width: 135, height: 135, text: frontDomains + '?id=' + this.pageId });
    },
    //diy详情
    getInfo() {
    ...
    arraySort() {
      ...
      tempArr.push(basis, marketing, member, tool);
