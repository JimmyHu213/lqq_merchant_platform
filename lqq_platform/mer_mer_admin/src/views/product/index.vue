...
import {proSearchFormItems} from "@/views/product/config";
...
export default {
  ...
  async mounted() {
    this.searchFormItems = await proSearchFormItems();
  ...
  methods: {
    ...
    onEdit(row,type) {
      //id:商品id，isDisabled：是否能编辑(1不能，2能)，isCopy：是否是采集商品(1是，2不是)
      if (this.tableFrom.type === '1' && type==='edit') {
        this.$modalSure('下架该商品吗？出售商品需下架之后可编辑。').then(() => {
          offShellApi(row.id).then(() => {
            this.$router.push({ path: `/product/list/creatProduct/${row.id}/2/2/${row.type}/${type}` });
          });
        });
      } else {
        this.$router.push({ path: `/product/list/creatProduct/${row.id}/2/2/${row.type}/${type}` });
    ...
    handleDelete(id, type) {
      this.$modalSure(type == 5 ? `删除 id 为 ${id} 的商品` : `id为${id}的商品加入回收站吗`).then(() => {
        ...
        productDeleteApi({
          ...
          this.$message.success(type == 5 ? '删除成功' : '加入回收站成功');
          this.delSuccess();
        });
      });
    },
