...
export default {
  ...
  created() {
    this.$watch('formValidate.attrList', this.watCh);
  ...
  methods: {
      ...
      productDetailApi(id)
        .then(async (res) => {
          ...
          this.formValidate = {
            ...
            attrValueList: info.attrValueList,
          ...
          if (info.specType) {
            info.attrValueList.forEach((row) => {
            ...
            this.multipleSelection = info.attrValueList;
          } else {
            this.ManyAttrValue = info.attrValueList;
            this.formValidate.attrList = [];
          }
          this.getShopImg(info.sliderImage, info.attrValueList);
        })
        .catch((res) => {
      ...
      draftInfoApi(id)
        .then(async (res) => {
          ...
          this.formValidate = {
            ...
            unitName: info.unitName,
            tempId: info.tempId,
            attrList: JSON.parse(info.attrList),
          ...
          if (info.specType) {
            this.ManyAttrValue = JSON.parse(info.attrValueList);
            this.$nextTick(() => {
    ...
    handleSubmit: Debounce(function (name) {
      if (!this.formValidate.specType) {
        // this.formValidate.attrList = []
        this.formValidate.attrValueList = this.ManyAttrValue;
      } else {
        // this.multipleSelection.forEach((row) => {
        //   this.$set(row, 'checked', true)
        // });
        this.formValidate.attrValueList = this.multipleSelection;
