...
export default {
  ...
  state: {
    ...
    // 已知组件列表默认数据 数组
    defaultArray: {},
    systemFormList: JSON.parse(localStorage.getItem('systemFormList')) || [] /** 系统表单列表 **/,
  },
  mutations: {
    FOOTER(state, data) {
     ...
     * @constructor
     */
    SET_SystemForm: (state, systemForm) => {
      state.systemFormList = systemForm;
      if (!systemForm.length) localStorage.removeItem('systemFormList');
    },
  },
  actions: {
    getData({ commit }, data) {},
