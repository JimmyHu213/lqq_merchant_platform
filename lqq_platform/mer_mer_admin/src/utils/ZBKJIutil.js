...
/** 商户信息 **/
export async function getMerchantConfigInfo() {
  try {
    let res = await merchantConfigInfoApi();
...
export function getServiceInfo() {
  try {
    const configStr = localStorage.getItem('JavaMerchantConfigInfo');
    if (!configStr) {
      getMerchantConfigInfo();
      return '';
    }
    const JavaMerchantConfigInfo = JSON.parse(configStr);
    if (!JavaMerchantConfigInfo) {
      getMerchantConfigInfo();
      return '';
    }
