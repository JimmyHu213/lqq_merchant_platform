<template>
  <div>
    <el-tabs v-model="typeDate" class="list-tabs" @tab-click="radioChange">
      ...
      <el-tab-pane v-if="modelName !== 'diy' && modelName ==='imagesAndVideos'" label="视频" name="video"></el-tab-pane>
    </el-tabs>
        ...
        <div class="conter mb15 relative">
          <div class="bnt" :style="{ paddingLeft: listType == 1 ? '17px' : '0' }">
            ...
            <div class="input">
              <div>
                <el-input
                  v-model.trim="tableData.keywords"
                  class="searchInput"
              ...
              >
                ...
                <div class="mt10 mb5">
                  <div
                      v-if="!item.isEdit"
                      class="imgTitle"
                      :style="{ width: pictureType ? `${(divWidth - 104) / 6}px` : '90px' }"
                  >
                    {{ item.alias }}
                  </div>
                  <el-input size="small" type="text" v-model="item.newName" v-else @blur="bindTxt(item)" clearable />
                </div>
                <!-- 删除/重命名/查看 -->
                <div class="operate" v-if="pictureType && item.isCk">
                  <span @click="deleteOperate(item)">删除</span>
                  <span class="ml10" @click="startEditName(item)">改名</span>
              ...
              >
                ...
                <el-table-column label="图片名称">
                  <template slot-scope="scope">
                    <div class="imgDiv">
                      <el-image :src="scope.row.sattDir" :preview-src-list="[scope.row.sattDir]" alt="" class="listImg" v-if="typeDate == 'pic'" />
                      ...
                      <el-input size="small" class="ml20" type="text" v-model="scope.row.newName" v-else @blur="bindTxt(scope.row)" clearable />
                    </div>
                  </template>
                </el-table-column>
                ...
                <el-table-column label="操作">
                  <template slot-scope="scope">
                    <a type="primary" @click="deleteListFun(scope)">删除</a>
                    <span class="decollator">|</span>
                    <a type="primary" @click="startEditName(scope.row)">重命名</a>
                  </template>
                </el-table-column>
              </el-table>
              <div class="block">
...
export default {
  ...
  created() {
    ...
    if (checkPermi(['platform:attachment:list'])) this.getFileListAsync();
  },
  mounted() {
    if (this.pictureType) {
  ...
  methods: {
    searchFile() {
      ...
      this.getFileListAsync();
    },
    // 开始编辑图片名称
    startEditName(item) {
      // 在进入编辑状态前，从别名中提取基础名称（去掉后缀）作为初始值
      if (item.alias && typeof item.alias === 'string') {
        // 移除可能存在的后缀，保留基础名称
        const baseName = item.alias.replace(/\.[^/.]+$/, '');
        // 设置输入框的初始值
        item.newName = baseName;
      }
      // 进入编辑状态
      item.isEdit = true;
      this.$nextTick(() => {
        // 使用更可靠的方式查找输入框 - 查找最近的el-input组件内的输入框
        const inputs = document.querySelectorAll('.el-input__inner');
        // 遍历所有输入框，找到第一个非空值或者最近激活的输入框
        for (let i = 0; i < inputs.length; i++) {
          // 优先选择有值且值与我们设置的newName匹配的输入框
          if (inputs[i].value === item.newName) {
            inputs[i].focus();
            inputs[i].select();
            return;
          }
        }
        // 如果找不到匹配值的输入框，就聚焦第一个可见的el-input
        for (let i = 0; i < inputs.length; i++) {
    ...
    bindTxt(item) {
      // 当用户输入名称后，提取名称（不含后缀）
      if (item.newName && typeof item.newName === 'string') {
        // 移除新输入名称中可能存在的后缀
        const baseName = item.newName.replace(/\.[^/.]+$/, '');

        // 从原始alias中提取文件后缀（如果有的话）
        let fileExtension = '.png'; // 默认后缀
        if (item.alias && typeof item.alias === 'string') {
          const extensionMatch = item.alias.match(/\.[^/.]+$/);
          if (extensionMatch && extensionMatch[0]) {
            fileExtension = extensionMatch[0]; // 获取原始文件后缀
          }
        }

        // 给别名添加原始文件后缀
        item.newName = baseName + fileExtension;
      }
      // 关闭编辑状态
      item.isEdit = false;
      this.saveName(item)
    ...
    async saveName(item){
      try {
        await attachmentNameUpdateApi({alias: item.newName, attId: item.attId})
        this.$message.success('操作成功');
        await this.getFileListAsync();
      }catch (e) {
        item.newName = item.alias
      }
    },
    //视口大小变化更改盒子宽度
    getWindowInfo() {
        ...
        fileListApi(this.tableData)
          .then(async (res) => {
            res.list.forEach((el) => {
              ...
              el.realName = false;
              el.num = 0;
              // 从别名中提取基础名称（去掉.png后缀）作为初始值
              el.newName = el.alias ? el.alias.replace(/\.[^/.]+$/, '') : '';
            });
    ...
    videoChange(val) {
      ...
      if (val == false) {
        this.$set(this.tableData, 'type', 'image');
        this.tableData.page = 1;
        this.getFileListAsync();
      } else {
        this.$set(this.tableData, 'type', 'video');
        this.tableData.page = 1;
        this.getFileListAsync();
      }
    },
