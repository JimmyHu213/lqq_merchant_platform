...
import { areasMerchantListApi } from '@/api/area';
...
export default {
  ...
  mounted() {
    if (checkPermi(['platform:merchant:page:list', 'circle:merchant:list'])) this.getMerList();
  },
  methods: {
    // 下拉加载更多
    selectLoadMore() {
    ...
    // 列表
    getMerList() {
      const api = isPlatform ? merchant.merchantListApi : areasMerchantListApi;
      api(this.search).then((res) => {
