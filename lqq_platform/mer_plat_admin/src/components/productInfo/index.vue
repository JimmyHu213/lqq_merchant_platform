      ...
      <div v-loading="loading">
        ...
        <el-tabs type="border-card" v-model="currentTab" v-if="formValidate.id && isShow">
          <el-tab-pane label="基本信息" name="0">
            <div class="detailSection divBox">
              <ul class="list mt-16">
                ...
                <li v-if="isDeliveryMethod && formValidate.deliveryMethod" class="item">
                  <div class="lang">配送方式：</div>
                  <template v-if="Number(formValidate.type) > 4 && Number(formValidate.type) != 8">
              ...
              <div class="list" style="display: block">
                <div class="item">
                  ...
                  <div class="value">{{ formValidate.keyword | filterEmpty }}</div>
                </div>
                ...
                <div class="item">
                  ...
                  <div class="value">{{ formValidate.intro | filterEmpty }}</div>
                </div>
                <div class="item">
                ...
                <div
                  class="item"
                  v-if="formValidate.systemFormValue && formValidate.type !== OrderSecondTypeEnum.Reservation"
                >
                  <div class="acea-row row-middle">
          ...
          <el-tab-pane label="规格库存" name="1">
            <div class="detailSection divBox">
              <ul class="list mt-16 mb16" v-if="formValidate.type == OrderSecondTypeEnum.PunchCard">
                <li class="item">
                  <div class="tips tipsWidth">使用次数：</div>
                  <div class="value">{{ formValidate.verifyQuantity }}</div>
                </li>
                <li class="item">
                  <div class="tips tipsWidth">有效期：</div>
                  <div class="value" v-if="formValidate.verifyTimeLimitType === 2">
                    购买后{{formValidate.verifyTimeLimitDay}}天内有效
                  </div>
                  <div class="value" v-else-if="formValidate.verifyTimeLimitType === 3">
                    {{formValidate.verifyTimeLimitStartDate}}~{{formValidate.verifyTimeLimitEndDate}}
                  </div>
              ...
              <ul class="list mt-16">
                ...
                <li v-show="formValidate.type >= 0" class="item">
                  <div class="tips tipsWidth">会员类型：</div>
                  <div class="value"> {{ vipTypeText }}</div>
                </li>
              </ul>
              <div style="margin-top: 16px">
                ...
                <template>
                  <el-table :data="AttrValueList" class="tabNumWidth" size="small">
                    ...
                    <el-table-column
                      v-if="formValidate.type >= 0 && formValidate.type !== 7"
                      label="商品条码"
                      width="100"
                    >
                      <template slot-scope="scope">
          ...
          <el-tab-pane label="预约设置" name="3" v-if="formValidate.type === OrderSecondTypeEnum.Reservation">
            <div class="detailSection divBox">
              <div class="list" style="display: block">
                <div class="item">
                  <div class="lang">服务模式：</div>
                  <div class="acea-row">
                    <div v-show="formValidate.deliveryMethod.includes('3')" style="color: #303133">到店服务</div>
                    <div
                      v-show="formValidate.deliveryMethod.includes('3') && formValidate.deliveryMethod.includes('4')"
                      style="color: #303133"
                    >
                      、
                    </div>
                    <div v-show="formValidate.deliveryMethod.includes('4')" style="color: #303133">上门服务</div>
                  </div>
                </div>
                <div class="item">
                  <div class="lang line-heightOne">剩余可约数量：</div>
                  <div class="value">
                    {{ formValidate.supplementReservation.remainingNumShouSwitch ? '显示' : '隐藏' }}
                  </div>
                </div>
                <div class="item">
                  <div class="lang">可约日期：</div>
                  <div class="value" v-if="formValidate.supplementReservation.reservationDateType === 1">每天</div>
                  <div class="value" v-else>
                    {{ formValidate.supplementReservation.reservationDateStartDate }} ~
                    {{ formValidate.supplementReservation.reservationDateEndDate }}的
                    {{ getWeekName(formValidate.supplementReservation.reservationDateWeekRange) }}
                  </div>
                </div>
                <div class="item">
                  <div class="acea-row row-middle">
                    <div class="lang">预约日期范围：</div>
                    <div class="value">
                      对用户展示{{ formValidate.supplementReservation.reservationDateDayRange }}天内的可预约日期
                    </div>
                  </div>
                </div>
                <div class="item">
                  <div class="acea-row row-middle">
                    <div class="lang">提前预约：</div>
                    <div class="value">
                      {{
                        formValidate.supplementReservation.advanceReservationSwitch
                          ? `用户需要提前${formValidate.supplementReservation.advanceReservationHour}小时进行预约`
                          : '无需提前预约'
                      }}
                    </div>
                  </div>
                ...
                <div class="item">
                  <div class="acea-row row-middle">
                    <div class="lang">取消预约：</div>
                    <div class="value">
                      {{
                        formValidate.supplementReservation.cancelOrderSwitch
                          ? `服务开始${formValidate.supplementReservation.cancelOrderHour}小时之前，允许取消并自动退款`
                          : '不允许用户取消预约'
                      }}
                    </div>
                  </div>
                ...
                <div class="item">
                  <div class="acea-row row-middle">
                    <div class="lang">表单信息：</div>
                    <div class="value">
                      {{
                        formValidate.supplementReservation.formDataSubmitType === 1
                            ? '每个预约提交1次'
                            : '每单提交1次'
                      }}
                    </div>
...
export default {
  ...
  computed: {
    // 配送方式判断
    isDeliveryMethod(){
      return (this.formValidate.type !== this.OrderSecondTypeEnum.Fictitious && this.formValidate.type !== this.OrderSecondTypeEnum.Reservation) || this.formValidate.type === this.OrderSecondTypeEnum.Normal
    },
    // 会员商品类型
    vipTypeText() {
      let text = '无'
      if (this.formValidate.vipPriceType == 1) {
        text = 'svip'
      } else if (this.formValidate.vipPriceType == 2) {
        text = '商户会员'
      }
      return text
    }
