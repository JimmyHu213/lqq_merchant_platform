        ...
        <el-card class="box-card" :bordered="false" shadow="never">
          ...
          >
            ...
            <template v-if="!selectModel">
              ...
              <el-table-column v-if="isPlatform" label="操作" :width="biztype.value === 1 ? 180 : 110" fixed="right">
                <template slot-scope="scope">
                  <template
                    v-if="
                      biztype.value === 1 &&
...
export default {
  ...
  mounted() {
    if (this.biztype.value === 2) {
      ...
      if (checkPermi(['platform:product:category:list'])) this.handlerGetTreeList();
    }
  },
  methods: {
    checkPermi, //权限控制
