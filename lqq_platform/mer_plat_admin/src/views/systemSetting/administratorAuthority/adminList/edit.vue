  ...
  <div>
    ...
    <div slot="footer" class="dialog-footer-inner">
      ...
      <el-button
        ...
        v-hasPermi="['platform:admin:save', 'platform:admin:update', 'circle:admin:save', 'circle:admin:update']"
      >
...
export default {
  ...
  methods: {
    ...
    handleGetRoleList() {
      ...
      // 根据isPlatform标识选择对应的API
      const getRoleListApi = isPlatform ? roleApi.getRoleList : areaApi.areasRoleListApi;

      getRoleListApi(_pram).then((data) => {
        this.roleList = data;
        let arr = [];
    ...
    handlerSave() {
      // 将roles数组转换为逗号分隔的字符串
      const submitData = { ...this.pram };
      submitData.roles = Array.isArray(this.pram.roles) ? this.pram.roles.join(',') : this.pram.roles;

      const api = isPlatform ? systemAdminApi.adminAdd : areaApi.areasAdminSaveApi;
    ...
    handlerEdit() {
      // 将roles数组转换为逗号分隔的字符串，避免直接修改原始数据
      const submitData = { ...this.pram };
      submitData.roles = Array.isArray(this.pram.roles) ? this.pram.roles.join(',') : this.pram.roles;

      const api = isPlatform ? systemAdminApi.adminUpdate : areaApi.areasAdminUpdateApi;
      api(submitData)
        .then((data) => {
        ...
        .catch(() => {
          // 错误处理，无需恢复原始数据因为我们使用了副本
          console.error('更新管理员失败');
        });
    },
