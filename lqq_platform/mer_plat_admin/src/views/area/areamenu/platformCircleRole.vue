      ...
      <el-form inline @submit.native.prevent>
        <el-form-item>
          <el-button
            ...
            v-hasPermi="['platform:admin:role:save', 'circle:role:save']"
            >添加角色</el-button
          >
        </el-form-item>
      </el-form>
      <el-table :data="listData.list" size="small">
        ...
        <el-table-column label="状态" prop="status" fixed="right" min-width="100">
          <template slot-scope="scope">
            <el-switch
              v-if="checkPermi(['platform:admin:role:update:status', 'circle:role:update:status'])"
        ...
        <el-table-column label="操作" width="100" fixed="right">
          <template slot-scope="scope">
            <a @click="handlerOpenEdit(1, scope.row)" v-hasPermi="['platform:admin:role:update', 'circle:role:update']"
              >编辑</a
            >
            <el-divider direction="vertical"></el-divider>
            <a @click="handlerOpenDel(scope.row)" v-hasPermi="['platform:admin:role:delete', 'circle:role:delete']"
              >删除</a
            >
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    handlerOpenDel(rowData) {
      this.$modalSure('删除当前数据').then(() => {
        areaApi.platformCircleRoleDeleteApi(rowData.id).then((data) => {
          this.$message.success('删除数据成功');
          // 判断是否是最后一页
          if (this.listData.list.length === 1 && this.listPram.page > 1) {
            this.listPram.page--;
          }
          this.handleGetRoleList();
        });
    ...
    handleGetRoleList() {
      areaApi.platformCircleRoleListApi(this.listPram).then((data) => {
        this.listData = data;
      });
    },
    ...
    handleStatusChange(row) {
      areaApi.platformCircleRoleUpdateStatusApi(row).then((res) => {
