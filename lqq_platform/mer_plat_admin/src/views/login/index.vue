    ...
    <div class="container" :class="[fullWidth > 768 ? 'containerSamll' : 'containerBig']">
      ...
      <div class="index_from page-account-container">
        <div class="labelPic">
          ...
          <img v-else src="../../assets/imgs/laber1.png" />
        </div>
...
import { areasGetLoginPicApi, areasLoginAccountDetectionApi } from '@/api/area';
import { accountDetectionApi, frontDomainApi, getSystemColorApi, getTencentMapKeyApi } from '@/api/systemConfig';
import { getChangeColorApi } from '@/api/publicly';
...
export default {
  ...
  methods: {
    ...
    async onBlurAccount() {
      if (this.loginForm.account) {
        if (this.isPlatform) {
          ...
          this.errorsNumber = await areasLoginAccountDetectionApi({ account: this.loginForm.account });
        }
      }
    },
    // 获取移动端域名
    ...
    //获取腾讯地图API Key
    getTencentMapKey() {
      getTencentMapKeyApi().then((res) => {
        this.$store.commit('settings/SET_tencentMapKey', res);
      });
    },
    handleResize(event) {
    ...
    success(params) {
      ...
      this.$store
        ...
        .then(() => {
          ...
          this.getTencentMapKey();
          this.$store
            .dispatch('user/getMenus', {
              that: this,
            })
            .then((res) => {
              if (this.__isPlatform) {
                this.$router.push({ path: '/dashboard' });
              } else {
                this.$router.push({ path: '/circle/dashboard' });
              }
            });
        })
        .catch(async (err) => {
