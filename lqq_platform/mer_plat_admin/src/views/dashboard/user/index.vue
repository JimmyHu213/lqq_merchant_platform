<template>
  <div>
    ...
    <!-- 统计数据 -->
    <div v-if="userStatisticsData.length && checkPermi(['platform:user:statistics:top'])" class="mt14">
...
export default {
  ...
  data() {
    return {
      ...
      newMemberLoading: true, // 新增付费会员图像是否加载中
      ...
      // 新增付费会员线形图配置
      newMemberOption: {
        title: { text: '新增付费会员数量', left: 0, top: 10, textStyle: { fontSize: 18, fontWeight: 600 } },
        // 图像位置
        grid: { top: 80, left: 0, right: 50, bottom: 40, containLabel: true },
        color: ['#5B8FF9'],
        // 指针数据显示
