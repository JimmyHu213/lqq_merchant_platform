<template>
  <div class="divBox">
    <el-card
      ...
      v-hasPermi="['platform:merchant:page:list', 'circle:merchant:list']"
    >
    ...
    <el-card class="box-card mt14" :body-style="{ padding: '0 20px 20px' }" shadow="never" :bordered="false">
      ...
      >
        ...
        <el-table-column prop="copyProductNum" label="剩余采集次数" min-width="110" />
        <el-table-column prop="sort" label="推荐指数" min-width="110" />
...
export default {
  ...
  mounted() {
    ...
    if (checkPermi(['platform:merchant:list:header:num', 'circle:merchant:header:num'])) this.getHeadNum();
    if (checkPermi(['platform:merchant:page:list', 'circle:merchant:list'])) this.getList('');
  },
  methods: {
    checkPermi,
    //详情中点击编辑按钮
    ...
    getHeadNum() {
      ...
      const api = isPlatform ? merchant.merHeaderNumApi : areaApi.areasMerchantListHeaderNumApi;
      api(data)
        .then((res) => {
