    ...
    <el-card class="box-card" shadow="never" :bordered="false">
      <el-form ref="pram" :model="pram" :rules="rules" label-width="100px">
        ...
        <el-form-item
          v-if="
            checkPermi([
              'platform:login:admin:update',
              'platform:login:admin:update:password',
              'circle:admin:update',
              'circle:admin:update:status',
            ])
          "
        >
...
export default {
  ...
  data() {
    ...
    const JavaInfo = isPlatform
      ? JSON.parse(Cookies.get('JavaPlatInfo'))
      : JSON.parse(Cookies.get('CircleJavaPlatInfo'));
    return {
      JavaInfo: isPlatform ? JSON.parse(Cookies.get('JavaPlatInfo')) : JSON.parse(Cookies.get('CircleJavaPlatInfo')),
      pram: {
  ...
  methods: {
    ...
    onSavePassword() {
      if (isPlatform) {
        ...
        areaApi.areasLoginAdminUpdatePasswordApi(this.pram).then((data) => {
          this.$message.success('提交成功');
        });
      }
    },
    ...
    onSaveAccount() {
      if (isPlatform) {
        ...
        areaApi.areasLoginAdminUpdateApi(this.pram).then((data) => {
          this.$message.success('提交成功');
        });
      }
    },
