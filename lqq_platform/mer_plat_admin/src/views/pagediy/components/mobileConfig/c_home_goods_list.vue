...
export default {
  ...
  methods: {
    getConfig(data) {
      ...
      let tab_radio = data.name === 'tab_radio' && (data.values <3)
      if (data.name && (tab_radio || data.name === 'merchantName' ||
          data.name === 'select' ||
          data.name === 'cascader' ||
          data.name === 'radio' ||
          data.name === 'number')) this.getProList();
    },
    //选择tab获取商品列表
    getProList() {
        let activeValue = '';
        ...
        productActivityListApi({
          categoryId: this.tabConfig === 0 ? activeValue : null,
          page: 1,
          limit: this.configObj.numConfig.val,
          type: '1',
          brandId: this.tabConfig === 1 ? activeValue : null,
          merIds: this.tabConfig === 2 ? activeValue : null,
