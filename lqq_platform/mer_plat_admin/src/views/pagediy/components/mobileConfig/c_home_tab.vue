  ...
  methods: {
    getConfig(data) {
      ...
      if (data.name && data.name === 'add_tab') {
        ...
        this.configObj.activeValueMer.activeValue = [];
        this.configObj.typeConfig.activeValue = [0, 1, 2];
        this.configObj.goodsSort.tabVal = 0; // 商品排序
        this.configObj.sortCollation.tabVal = 0; //排序规则
        this.getProList();
      }
      //选择tab获取商品列表.0指定商品
      ...
      //选择tab获取商品列表,0指定分类，1指定品牌，2指定商户
      let tab_radio = data.name === 'tab_radio' && this.configObj.tabConfig.tabVal < 3;
      if (
        data.name &&
        (tab_radio ||
    ...
    getProList() {
      ...
      productActivityListApi({
        categoryId: this.configObj.tabConfig.tabVal === 0 ? activeValue : null,
        page: 1,
        limit: this.configObj.numConfig.val,
        type: '1',
        brandId: this.configObj.tabConfig.tabVal === 1 ? activeValue : null,
        merIds: this.configObj.tabConfig.tabVal === 2 ? activeValue : null,
        ...
        switch (this.configObj.tabConfig.tabVal) {
          case 0:
            ...
            break;
          case 1:
            this.configObj.tabItemConfig.list[this.configObj.tabItemConfig.tabVal].brandGoodsList = res.list;
            break;
          case 2:
            this.configObj.tabItemConfig.list[this.configObj.tabItemConfig.tabVal].merList = res.list;
            break;
        }
        this.setProConfig();
      });
    ...
    setProConfig() {
      ...
      this.configObj.tabItemConfig.list[this.configObj.tabItemConfig.tabVal].activeList = {
        ...
        activeProTabIndex: this.configObj.tabConfig.tabVal, //商品列表类型，商品、分类、品牌、商户
