<template>
  <div class="page-container" v-if="configObj">
    ...
    <div class="search-box">
      <template v-if="configObj.searConfig.tabVal === 1">
        <img :src="logoUrl" alt="" v-if="configObj.logoConfig.isShow === 1 && logoUrl" mode="widthFix" />
      </template>
     <template v-if="configObj.searConfig.tabVal === 2">
       <div class="acea-row map row-middle">
         <span v-show="configObj.searchConfig.tabVal===0" class="iconfont icon-dingwei1 font14"></span>
         <span class="ml2">国润城</span>
         <span v-show="configObj.searchConfig.tabVal===1" class="iconfont icon-caozuo-xia font14 ml2"></span>
       </div>
     </template>
          ...
          <div>
            <div class="dot more-dot" :style="dotStyle" v-if="docType === 0">
              <div
                class="dot-item"
                :style="{
                  background: themeStyle ? configObj.docColor.color[0].item : themeColor,
                }"
              ></div>
...
export default {
  ...
  mounted() {
    this.fromTo = this.$route.params.fromTo
    this.$nextTick(() => {
