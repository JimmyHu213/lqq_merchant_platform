  ...
  <div class="deviseBox">
    <div class="devise_head acea-row row-between-wrapper">
      ...
      <div class="acea-row preview">
        <el-popover placement="top-start" trigger="hover" :disabled="pageId === 0">
          <div id="diyQrcode"></div>
          <el-button :disabled="pageId === 0" class="ht_btn" slot="reference" style="line-height: 9px"
              ...
              <div class="border-box">
                <div class="scrollCon">
                  <div style="margin: 0 auto; position: relative; height: auto">
                    ...
                    >
                      <div class="overflowy">
                        <div class="picture"><img src="@/assets/imgs/title.png" /></div>
                      </div>
                      <draggable
                      ...
                      >
                        <div
                          ...
                          :class="{
                            on: activeIndex == key,
                            mt40: item.name === 'search_box' || item.name === 'home_comb',
                            top:
                        ...
                        >
                          ...
                          <div class="delete-box">
                            <div class="handleType" :style="item.name === 'home_footer' ? 'top: -94px; ' : ''">
                              ...
                              <div
                                class="iconfont"
                                :class="item.isHide ? 'icon-caozuo-yincang' : 'icon-caozuo-xianshi'"
...
export default {
  ...
  methods: {
    ...
    // 区域diy判断 头部组件与搜索框必须添加任一个
    checkItemExists(items) {
      // 检查是否存在具有特定name的对象，只要含有其中一个，就返回true
      return Object.keys(items).some((key) => {
        return items[key].name === 'homeComb' || items[key].name === 'headerSerch';
      });
    },
    getDiySaveDate(val, n, j) {
      let data = {
        value: val,
        title: this.titleTxt,
    ...
    //提交数据
    async diySaveDate(val, n, j) {
      let data = this.getDiySaveDate(val, n, j);
      if (!data.name) return this.$message.warning('模板名称不能为空');
      if (this.pageId === 0 || j === 1 || (this.pageType === 'copy' && Number(this.$route.params.id) === this.pageId)) {
        try {
          // 新增
        ...
        // 编辑
        try {
          const res = await pagediyUpdateApi(data);
    ...
    arraySort() {
      ...
      this.lConfig.map((el, index) => {
        if (this.fromTo === 'regionHome') {
          if (el.name !== 'home_tab' && el.type == 0) {
            basis.list.push(el);
          }
          if (el.name !== 'home_intergral' && el.name !== 'home_group' && el.name !== 'home_seckill' && el.type == 1) {
