<template>
  <div class="divBox relative">
    <el-card
      ...
      v-hasPermi="['platform:order:page:list', 'circle:order:list']"
    >
    ...
    >
      ...
      >
          ...
          <template slot-scope="scope">
            ...
            <span
              ...
              v-else-if="
                ...
                scope.row.status == 3 &&
                scope.row.verifyRemainingTimes != scope.row.verifyTotalTimes
              "
              >部分使用</span
            >
            <span
        ...
        <el-table-column v-if="checkPermi(['platform:order:info', 'circle:order:info'])" width="65" fixed="right">
...
export default {
  ...
  mounted() {
    if (checkPermi(['platform:order:page:list', 'circle:order:list'])) this.getList();
    if (checkPermi(['platform:order:status:num', 'circle:order:status:num'])) this.getOrderStatusNum();
  },
  methods: {
    checkPermi,
    getMerId(id) {
    ...
    getList() {
      ...
      if (isPlatform) {
        ...
        areaApi
          .areasOrderListApi(this.tableFrom)
          .then((res) => {
            this.tableData.data = res.list || [];
            this.tableData.total = res.total;
            this.listLoading = false;
    ...
    getOrderStatusNum() {
      ...
      if (isPlatform) {
        ...
        areaApi.areasOrderStatusNumApi(data).then((res) => {
          this.orderChartType = res;
        });
      }
    },
