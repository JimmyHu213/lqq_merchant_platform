<template>
  <div class="divBox relative">
    <el-card
      ...
      v-if="checkPermi(['platform:refund:order:status:num', 'circle:refund:order:status:num'])"
    >
    ...
    >
      ...
      >
        ...
        <el-table-column width="160" fixed="right">
          ...
          <template slot-scope="scope">
            <a
              v-hasPermi="['platform:refund:order:detail', 'circle:refund:order:detail']"
...
export default {
  ...
  mounted() {
    if (checkPermi(['platform:refund:order:page:list', 'circle:refund:order:list'])) this.getList();
    if (checkPermi(['platform:refund:order:status:num', 'circle:refund:order:status:num'])) this.getOrderStatusNum();
  },
  methods: {
    checkPermi,
    getMerId(id) {
    ...
    getList() {
      ...
      if (isPlatform) {
        ...
        areaApi
          .areasRefundOrderListApi(this.tableFrom)
          .then((res) => {
            this.tableData.data = res.list || [];
            this.tableData.total = res.total;
            this.listLoading = false;
    ...
    getOrderStatusNum() {
      ...
      if (isPlatform) {
        ...
        areaApi.areasRefundOrderStatusNumApi(data).then((res) => {
          this.orderChartType = res;
        });
      }
    },
