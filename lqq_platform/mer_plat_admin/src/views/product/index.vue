<template>
  <div class="divBox relative">
    <el-card
      ...
      v-hasPermi="['platform:product:page:list', 'circle:merchant:list']"
    >
      <div class="padding-add">
        <el-form inline label-position="right" @submit.native.prevent>
          ...
          <el-form-item label="会员类型：">
            <el-select
              v-model="tableFrom.vipPriceType"
              clearable
              size="small"
              placeholder="请选择"
              class="selWidth"
              @change="handleSeachList"
            >
              <el-option label="SVIP" value="1" />
              <el-option label="商户会员" value="2" />
          ...
          <el-form-item>
            <el-button
              type="primary"
              size="small"
              @click="handleSeachList"
              v-hasPermi="['platform:product:page:list', 'circle:product:list']"
              >查询</el-button
            >
    ...
    <el-card class="box-card mt14" :body-style="{ padding: '0 20px 20px' }" shadow="never" :bordered="false">
      ...
      <div class="acea-row mt5 table-btn-group-box">
        <el-button
          size="small"
        ...
        <el-button
          ...
          >增加初始销量</el-button
        >
        <div class="preview-text-box" v-if="tableFrom.type === ProductTypeEnum.OnSale">
      ...
      >
        <el-table-column
          v-if="(Number(tableFrom.type) < 3 || tableFrom.type === ProductTypeEnum.Audit) && isPlatform"
          key="2"
          type="selection"
          width="55"
        />
        ...
        <el-table-column prop="id" label="ID" width="50" v-if="checkedCities.includes('ID') && isPlatform" />
        ...
        <el-table-column label="操作" width="190px" fixed="right" :scoped-slot="renderHeader">
          <template slot-scope="scope">
            <a @click="handleAudit(scope.row.id, false)" v-hasPermi="['platform:product:info', 'circle:product:info']"
              >详情</a
            >
  ...
  methods: {
    ...
    goodHeade() {
      ...
      if (isPlatform) {
        ...
        areaApi
          .areasProductTabsHeadersApi(data)
          .then((res) => {
            res.map((item, index) => {
              if (item.type === headerName[index].type) item.name = headerName[index].name;
            });
    ...
    getList(num) {
      ...
      if (isPlatform) {
        ...
        areaApi
          .areasProductListApi(this.tableFrom)
          .then((res) => {
            this.tableData.data = res.list;
            this.tableData.total = res.total;
            this.listLoading = false;
    ...
    reset() {
      ...
      this.tableFrom.vipPriceType = null;
