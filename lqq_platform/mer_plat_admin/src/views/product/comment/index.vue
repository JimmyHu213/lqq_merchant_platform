<template>
  <div class="divBox">
    <el-card
      ...
      v-hasPermi="['platform:product:reply:list', 'circle:product:reply:list']"
    >
      <div class="padding-add">
        <el-form :inline="true" label-position="right" @submit.native.prevent>
          ...
          <el-form-item label="商户名称：" v-hasPermi="['platform:merchant:page:list', 'circle:merchant:list']">
    ...
    <el-card class="box-card mt16" shadow="never" :bordered="false">
      <el-table :data="tableData.data" size="small">
        ...
        <el-table-column label="操作" width="70" fixed="right" v-if="isPlatform">
          <template slot-scope="scope">
...
export default {
  ...
  mounted() {
    if (checkPermi(['platform:product:reply:list', 'circle:product:reply:list'])) this.getList(1);
  },
  methods: {
    checkPermi,
    remoteMethod(query) {
    ...
    getList(num) {
      ...
      const api = isPlatform ? replyListApi : areaApi.areasProductReplyListApi;
      api(this.tableFrom)
        .then((res) => {
