<template>
  ...
  >
    <img v-if="platMerLoginInfo.leftTopLogo" class="layout-logo-medium-img" :src="platMerLoginInfo.leftTopLogo" />
  </div>
  <div class="layout-logo-size" v-else @click="onThemeConfigChange">
    <img
      v-if="platMerLoginInfo.leftSquareLogo && $store.state.themeConfig.themeConfig.isCollapse"
      class="layout-logo-size-img"
      :src="platMerLoginInfo.leftSquareLogo"
    />
    <img
      v-else-if="platMerLoginInfo.leftTopLogo"
      class="layout-logo-size-img"
      :src="platMerLoginInfo.leftTopLogo"
    />
  </div>
</template>
...
import { isPlatform } from '@/utils/settingMer.js';
export default {
  name: 'layoutLogo',
  computed: {
    //登录后的信息
  ...
  methods: {
    ...
    onThemeConfigChange() {
      ...
      this.$router.push(isPlatform ? '/dashboard' : '/circle/dashboard');
    },
  },
};
</script>
